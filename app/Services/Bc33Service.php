...
class Bc33Service
{
    ...
    public function updateDraft($id, Request $request)
    {
        ...
        try {
            ...
            $barangInput = array_values(array_filter($request->input('barang', []), function ($b) {
                return !empty($b['uraian']) || !empty($b['posTarif']) || !empty($b['kodeBarang']);
            }));
            ...
            return redirect()->route('dokumen-pabean-index')
                            ->with('success', 'Draft BC 3.3 berhasil disimpan!');

        } catch (\Exception $e) {
            DB::connection('mysql_sb')->rollBack();
            Log::error('Error Update Draft BC 3.3: ' . $e->getMessage());

            return redirect()->back()
                            ->withInput()
                            ->with('error', 'Gagal menyimpan draft BC 3.3: ' . $e->getMessage());
        }
    }
    ...
    public function sendCeisa($id, Request $request)
    {
        ...
        try {
            ...
            $bel = reset($pembeli);
            if ($bel && (!empty($bel['namaEntitas']) || !empty($bel['nomorIdentitas']) || !empty($bel['kodeNegara']))) {
            ...
            $pen = reset($penerima);
            if ($pen && (!empty($pen['namaEntitas']) || !empty($pen['nomorIdentitas']) || !empty($pen['kodeNegara']))) {
            ...
            foreach (($draft['barang'] ?? []) as $index => $brg) {
                ...
                $itemBarang = [
                    ...
                    'nomorDaftarDokAsal'   => !empty($brg['nomorDaftar'])
                                                ? $brg['nomorDaftar']
                                                : ($item->bcno_in ?? $item->kpno ?? ''),
                    'tanggalDaftarDokAsal' => !empty($brg['tanggalDaftar'])
                                                ? $brg['tanggalDaftar']
                                                : ((!empty($item->tgl_bc_in) && $item->tgl_bc_in !== '0000-00-00')
                                                    ? $item->tgl_bc_in : ''),
            ...
            $finalPayload = [
                ...
                'kodeTps'               => $draft['kodeTps'] ?? $draft['kodeLokasiTps'] ?? '',
            ...
            Log::info('Kirim BC 3.3 CEISA Payload: ', $finalPayload);
            $responseCeisa = $this->ceisaService->kirimDokumenBc33($finalPayload);

            if ($responseCeisa['successful']) {


                $db->transaction(function () use ($db, $id, $nomorAju, $draft) {
                    $db->table('bppb')
                        ->where(function ($query) use ($id) {
            ...
            Log::error('Error Send CEISA BC 3.3: ' . $e->getMessage() . ' di baris ' . $e->getLine());
            return response()->json([
                'status'  => 500,
    ...
    public function editBatch($ids, Request $request)
    {
        ...
        return view('export-import.dokumen-pabean.edit-batch-bc33', [
            "page"           => "dashboard-export-import",
            "subPageGroup"   => "export-import",
            "subPage"        => "dokumen-pabean-list",
            "containerFluid" => true,
            ...
            "nomorAju"       => $nomorAju,
            "referensiDokumenAsal" => $referensiDokumenAsal,
            "kantorList"     => \App\Services\BcReferenceService::getKantorPelayanan(),
            "listJenisTpb"       => \App\Services\BcReferenceService::getJenisTpb(),
            "listTujuanPengiriman"=> \App\Services\BcReferenceService::getTujuanPengiriman('33'),
    ...
    public function updateDraftBatchBc33($ids, Request $request)
    {
        DB::connection('mysql_sb')->beginTransaction();
        ...
        try {
            ...
            // --- DOKUMEN & UPLOAD FILE (Diambil dari single) ---
            $dokumenInput = $request->input('dok', []);
            $dokumenFiles = $request->file('dok', []);
            $dokumenList  = [];
            $seriDok = 1;
            ...
            foreach ($dokumenInput as $index => $d) {
                if (!empty($d['kode']) || !empty($d['nomor'])) {
                    $dokData = $d; // Pertahankan input yang ada
                    $dokData['seriDokumen'] = $seriDok++;

                    if (isset($dokumenFiles[$index]['file_lampiran'])) {
                        $file = $dokumenFiles[$index]['file_lampiran'];
                        $fileName = 'CEISA_BATCH_' . str_replace('/', '-', $bpbs[0]) . '_' . ($d['kode'] ?? 'DOC') . '_' . time() . '.' . $file->getClientOriginalExtension();
                        $destinationPath = public_path('uploads/ceisa');
                        if (!file_exists($destinationPath)) {
                            mkdir($destinationPath, 0755, true);
                        }
            ...
            // --- BANK DEVISA ---
            $bankDevisaInput = $request->input('bankDevisa', []);
            $bankDevisaList = array_values(array_filter($bankDevisaInput, function ($b) {
                return !empty($b['kodeBank']) || !empty($b['namaBank']);
            }));
            ...
            // --- PENGANGKUT ---
            $pengangkutList = array_map(function ($p) {
                $p['seriPengangkut'] = (int) ($p['seriPengangkut'] ?? 0);
                return $p;
            }, $request->input('pengangkut', []));

            // --- ENTITAS ---
            $entitasList = $this->normalizeEntitasBc33($request->input('entitas', []));

            // --- KESIAPAN BARANG (Injeksi dari single) ---
            $kesiapanBarangList = array_values($request->input('kesiapanBarang', []));

            $ndpbmValue = (float) $request->input('ndpbm', 0) <= 0
                && $request->input('kodeValuta', 'IDR') === 'IDR'
                    ? 1
                    : (float) $request->input('ndpbm', 0);

            // --- BARANG ---
            $barangList = array_values(array_map(function ($brg) use ($ndpbmValue) {
                $brg['seriBarang']       = (int) ($brg['seriBarang'] ?? 0);
                $brg['cif']              = (float) ($brg['cif'] ?? 0);
                $brg['cifRupiah']        = (float) ($brg['cifRupiah'] ?? 0);
                if ($brg['cifRupiah'] <= 0 && $ndpbmValue > 0) {
                    $brg['cifRupiah'] = $brg['cif'] * $ndpbmValue;
                }
                $brg['fob']              = (float) ($brg['fob'] ?? 0);
                ...
                if (isset($brg['dokFasilitas'])) {
                    $brg['dokFasilitas'] = array_values($brg['dokFasilitas']);
                } else {
                    $brg['dokFasilitas'] = [];
                }

                if (isset($brg['entitasBarang'])) {
                    $brg['entitasBarang'] = array_values($brg['entitasBarang']);
                } else {
                    $brg['entitasBarang'] = [];
                }
            ...
            $draftPayload = [
                ...
                'nilaiPph'             => (float) $request->input('nilaiPph', 0),
                'totalDanaSawit'       => (float) $request->input('totalDanaSawit', 0),
                'kodeBendera'          => $request->input('kodeBendera', ''),
                'caraPengangkutanLainnya' => $request->input('caraPengangkutanLainnya', ''),
                ...
                'flagCurah'            => $request->input('flagCurah', ''),
                'kodeCaraAngkutPlb'    => $request->input('kodeCaraAngkutPlb', ''),
                'kodeCaraBayar'        => $request->input('kodeCaraBayar', ''),
                'kodeCaraDagang'       => $request->input('kodeCaraDagang', ''),
                'kodeJenisEkspor'      => $request->input('kodeJenisEkspor', ''),
            ...
            foreach ($bpbs as $id) {
                ...
                $header = $db->table('bppb')->select('bppbno_int')->where(function ($q) use ($id) {
                    $q->where('bppbno', $id)->orWhere('bppbno_int', $id);
                })->first();

                $bppbno_int = $header ? $header->bppbno_int : null;
            ...
            return back()->with('success', 'Draft Batch BC 3.3 berhasil disimpan untuk ' . count($bpbs) . ' dokumen!');

        } catch (\Exception $e) {
            DB::connection('mysql_sb')->rollBack();
            Log::error('Error Update Draft Batch BC 3.3: ' . $e->getMessage(), ['trace' => $e->getTraceAsString()]);
            return response()->json([
                'status'  => 500,
                'message' => 'Gagal menyimpan draft batch BC 3.3: ' . $e->getMessage(),
            ], 500);
        }
    }


    private function normalizeEntitasBc33(array $entitasInput): array
    {
        $result = [];

        foreach ($entitasInput as $kodeKey => $row) {
            if (!is_array($row)) {
                continue;
            }


            $isMultiRow = isset($row[0]) && is_array($row[0]);
    ...
    public function sendCeisaBatch33(array $bpbs, Request $request)
    {
        $db = DB::connection('mysql_sb');
        $firstBpb = $bpbs[0];
        ...
        try {
            ...
            if (!$header) throw new \Exception('Data transaksi tidak ditemukan di bppb!');
            ...
            if (empty($nomorAju)) throw new \Exception('Nomor Aju tidak ditemukan. Silakan Simpan Draft ulang.');

            // AMBIL ITEM BPPB UNTUK DOKUMEN ASAL
            $bppbItems = $db->table('bppb as a')
                ->where(function ($query) use ($firstBpb) {
                    $query->where('a.bppbno', $firstBpb)->orWhere('a.bppbno_int', $firstBpb);
                })
            ...
            foreach (($draft['barang'] ?? []) as $index => $brg) {
                ...
                $itemBarang = [
                    ...
                    "kodeDaerahAsal"    => strval($brg['kodeDaerahAsal'] ?? $draft['kodeDaerahAsal'] ?? '-'),
                    "kodeDokumen"       => "33",
                    "kodeJenisKemasan"  => strval($brg['kodeJenisKemasan'] ?? 'CT'),
                    "kodeNegaraAsal"    => strval($brg['kodeNegaraAsal'] ?? 'ID'),
                    ...
                    "dokFasilitas"         => $barangDokumen,
                    "entitasBarang"        => $entitasBarang,
                    "kodeKantorAsal"       => !empty($brg['kodeKantor']) ? $brg['kodeKantor'] : ($draft['kodeKantor'] ?? '050500'),
                    "kodeDokAsal"          => $brg['kodeJenisDokAsal'] ?? $brg['dokumenAsal'] ?? '',
                    "nomorAjuDokAsal"      => $brg['nomorPengajuan'] ?? '',
                    "nomorDaftarDokAsal"   => !empty($brg['nomorDaftar']) ? $brg['nomorDaftar'] : ($item->bcno_in ?? $item->kpno ?? ''),
                    "tanggalDaftarDokAsal" => !empty($brg['tanggalDaftar']) ? $brg['tanggalDaftar'] : ((!empty($item->tgl_bc_in) && $item->tgl_bc_in !== '0000-00-00') ? $item->tgl_bc_in : ''),
                    "seriBarangDokAsal"    => !empty($brg['seriBarangAsal']) ? $brg['seriBarangAsal'] : ($item->no_urut ?? ($index + 1)),
                ];
            ...
            $entitasByKode = [];
            foreach ($entitasDraft as $ent) {
                if (!empty($ent['kodeEntitas'])) {
                    $entitasByKode[(string) $ent['kodeEntitas']] = $ent;
                }
            }

            // Entitas yang sudah ada (tidak diubah struktur aslinya)
            $ent2 = $entitasByKode['2'] ?? [];
            $ent3 = $entitasByKode['3'] ?? [];
            $ent6 = $entitasByKode['6'] ?? [];
            $ent8 = $entitasByKode['8'] ?? [];

            // Entitas tambahan dari single
            $ent4  = $entitasByKode['4'] ?? [];
            $ent7  = $entitasByKode['7'] ?? [];
            $ent23 = $entitasByKode['23'] ?? [];

            $payloadEntitas = [
                [
                    "alamatEntitas"      => !empty($ent2['alamatEntitas']) ? strval($ent2['alamatEntitas']) : "-",
                    "kodeEntitas"        => "2",
                    "kodeJenisIdentitas" => !empty($ent2['kodeJenisIdentitas']) ? strval($ent2['kodeJenisIdentitas']) : "5",
                    "kodeStatus"         => !empty($ent2['kodeStatus']) ? strval($ent2['kodeStatus']) : "6",
                    "namaEntitas"        => !empty($ent2['namaEntitas']) ? strval($ent2['namaEntitas']) : "-",
                    "nibEntitas"         => !empty($ent2['nibEntitas']) ? strval($ent2['nibEntitas']) : "",
                    "nomorIdentitas"     => !empty($ent2['nomorIdentitas']) ? strval($ent2['nomorIdentitas']) : "-",
                    "nomorIjinEntitas"   => !empty($ent2['nomorIjinEntitas']) ? strval($ent2['nomorIjinEntitas']) : "-",
                    "seriEntitas"        => 1,
                    "tanggalIjinEntitas" => !empty($ent2['tanggalIjinEntitas']) ? strval($ent2['tanggalIjinEntitas']) : date('Y-m-d'),
                ],
                [
                    "alamatEntitas"      => !empty($ent3['alamatEntitas']) ? strval($ent3['alamatEntitas']) : "-",
                    "kodeEntitas"        => "3",
                    "kodeJenisIdentitas" => !empty($ent3['kodeJenisIdentitas']) ? strval($ent3['kodeJenisIdentitas']) : "5",
                    "namaEntitas"        => !empty($ent3['namaEntitas']) ? strval($ent3['namaEntitas']) : "-",
                    "nibEntitas"         => !empty($ent3['nibEntitas']) ? strval($ent3['nibEntitas']) : "",
                    "nomorIdentitas"     => !empty($ent3['nomorIdentitas']) ? strval($ent3['nomorIdentitas']) : "-",
                    "nomorIjinEntitas"   => !empty($ent3['nomorIjinEntitas']) ? strval($ent3['nomorIjinEntitas']) : "-",
                    "seriEntitas"        => 2,
                    "tanggalIjinEntitas" => !empty($ent3['tanggalIjinEntitas']) ? strval($ent3['tanggalIjinEntitas']) : date('Y-m-d'),
                ],
                [
                    "alamatEntitas"      => !empty($ent6['alamatEntitas']) ? strval($ent6['alamatEntitas']) : (!empty($header->alamat_supplier) ? strval($header->alamat_supplier) : "-"),
                    "kodeEntitas"        => "6",
                    "kodeNegara"         => !empty($ent6['kodeNegara']) ? strval($ent6['kodeNegara']) : "",
                    "namaEntitas"        => !empty($ent6['namaEntitas']) ? strval($ent6['namaEntitas']) : (!empty($header->supplier) ? strval($header->supplier) : "-"),
                    "nibEntitas"         => !empty($ent6['nibEntitas']) ? strval($ent6['nibEntitas']) : "",
                    "nomorIdentitas"     => !empty($ent6['nomorIdentitas']) ? strval($ent6['nomorIdentitas']) : "-",
                    "seriEntitas"        => 3,
                ],
                [
                    "alamatEntitas"  => !empty($ent8['alamatEntitas']) ? strval($ent8['alamatEntitas']) : (!empty($header->alamat_supplier) ? strval($header->alamat_supplier) : "-"),
                    "kodeEntitas"    => "8",
                    "kodeNegara"     => !empty($ent8['kodeNegara']) ? strval($ent8['kodeNegara']) : "",
                    "namaEntitas"    => !empty($ent8['namaEntitas']) ? strval($ent8['namaEntitas']) : (!empty($header->supplier) ? strval($header->supplier) : "-"),
                    "nibEntitas"     => !empty($ent8['nibEntitas']) ? strval($ent8['nibEntitas']) : "",
                    "nomorIdentitas" => !empty($ent8['nomorIdentitas']) ? strval($ent8['nomorIdentitas']) : "-",
                    "seriEntitas"    => 4,
                ],
            ];

            // Injeksi PPJK, Pemilik, dan Konsolidator jika ada
            $seriEnt = 5;
            if (!empty($ent4['nomorIdentitas'])) {
                $payloadEntitas[] = [
                    "alamatEntitas"      => $ent4['alamatEntitas'] ?? "-",
                    "kodeEntitas"        => "4",
                    "kodeJenisIdentitas" => $ent4['kodeJenisIdentitas'] ?? "5",
                    "namaEntitas"        => $ent4['namaEntitas'] ?? "-",
                    "nibEntitas"         => $ent4['nibEntitas'] ?? "",
                    "nomorIdentitas"     => $ent4['nomorIdentitas'],
                    "seriEntitas"        => $seriEnt++,
                ];
            }

            // Fallback pemilik ke eksportir sama seperti fungsi single
            if (empty($ent7['nomorIdentitas']) && !empty($ent2['nomorIdentitas'])) {
                $ent7 = $ent2;
            }
            if (!empty($ent7['nomorIdentitas'])) {
                $payloadEntitas[] = [
                    "alamatEntitas"      => $ent7['alamatEntitas'] ?? "-",
                    "kodeEntitas"        => "7",
                    "kodeJenisIdentitas" => $ent7['kodeJenisIdentitas'] ?? "5",
                    "namaEntitas"        => $ent7['namaEntitas'] ?? "-",
                    "nibEntitas"         => $ent7['nibEntitas'] ?? "",
                    "nomorIdentitas"     => $ent7['nomorIdentitas'],
                    "seriEntitas"        => $seriEnt++,
                ];
            }

            if (!empty($ent23['nomorIdentitas'])) {
                $payloadEntitas[] = [
                    "alamatEntitas"      => $ent23['alamatEntitas'] ?? "-",
                    "kodeEntitas"        => "23",
                    "kodeJenisIdentitas" => $ent23['kodeJenisIdentitas'] ?? "5",
                    "namaEntitas"        => $ent23['namaEntitas'] ?? "-",
                    "nibEntitas"         => $ent23['nibEntitas'] ?? "",
                    "nomorIdentitas"     => $ent23['nomorIdentitas'],
                    "seriEntitas"        => $seriEnt++,
                ];
            }
            ...
            foreach (($draft['bankDevisa'] ?? []) as $b) {
                if (!empty($b['kodeBank']) || !empty($b['namaBank'])) {
                    $payloadBankDevisa[] = [
                        ...
                        "seriBank"  => (int) ($b['seriBank'] ?? $seriBank),
                    ];
                    $seriBank++;
                }
            }

            // --- KESIAPAN BARANG (Diambil dari single) ---
            $kesiapanBarangList = [];
            foreach (($draft['kesiapanBarang'] ?? []) as $kb) {
                $waktuStr = $kb['waktuSiapPeriksa'] ?? date('Y-m-d\TH:i');
                $waktu = \Carbon\Carbon::parse($waktuStr)->format('Y-m-d\TH:i:s');
            ...
            // --- PENGANGKUT (Disesuaikan loop seperti single) ---
            $payloadPengangkut = [];
            $seriPengangkut = 1;
            foreach (($draft['pengangkut'] ?? []) as $p) {
                $payloadPengangkut[] = [
                    ...
                    "kodeCaraAngkut"  => !empty($p['kodeCaraAngkut']) ? strval($p['kodeCaraAngkut']) : "1",
                    "kodeBendera"     => !empty($p['kodeBendera']) ? strval($p['kodeBendera']) : "",
                    "seriPengangkut"  => (int) ($p['seriPengangkut'] ?? $seriPengangkut++),
                ];
            }
            ...
            $payload = [
                ...
                "kodeCaraAngkutPlb"   => !empty($draft['kodeCaraAngkutPlb']) ? strval($draft['kodeCaraAngkutPlb']) : "-",
                "kodeCaraBayar"       => !empty($draft['kodeCaraBayar']) ? strval($draft['kodeCaraBayar']) : "-",
                "kodeCaraDagang"      => !empty($draft['kodeCaraDagang']) ? strval($draft['kodeCaraDagang']) : "-",
                "kodeDaerahAsal"      => !empty($draft['kodeDaerahAsal']) ? strval($draft['kodeDaerahAsal']) : "",
                "kodeDokumen"         => "33",
                "kodeIncoterm"        => !empty($draft['kodeIncoterm']) ? strval($draft['kodeIncoterm']) : "-",
                "kodeJenisEkspor"     => !empty($draft['kodeJenisEkspor']) ? strval($draft['kodeJenisEkspor']) : "-",
                "kodeJenisProsedur"   => !empty($draft['kodeJenisProsedur']) ? strval($draft['kodeJenisProsedur']) : "-",
                "kodeJenisTpb"        => !empty($draft['jenisTpb']) ? strval($draft['jenisTpb']) : "1",
                ...
                "kodeKategoriEkspor"  => !empty($draft['kodeKategoriEkspor']) ? strval($draft['kodeKategoriEkspor']) : "-",
                "kodePelBongkar"      => !empty($draft['kodePelBongkar']) ? strval($draft['kodePelBongkar']) : "-",
                "kodePelMuat"         => !empty($draft['kodePelMuat']) ? strval($draft['kodePelMuat']) : "-",
                "kodePelTujuan"       => !empty($draft['kodePelTujuan']) ? strval($draft['kodePelTujuan']) : "-",
                "kodeTps"             => !empty($draft['kodeTps']) ? strval($draft['kodeTps']) : "UTPK",
                "kodeTujuanPengiriman"=> !empty($draft['kodeTujuanPengiriman']) ? strval($draft['kodeTujuanPengiriman']) : "1",
                ...
                "ndpbm"               => (float) ($draft['ndpbm'] ?? 0),
                "netto"               => (float) ($draft['netto'] ?? 0),
                "nik"                 => !empty($draft['nik']) ? strval($draft['nik']) : (!empty($ent2['nomorIdentitas']) ? strval($ent2['nomorIdentitas']) : "0000000000000000"),
                "nilaiBarang"         => $nilaiBarangHeader,
                "nilaiJasa"           => (float) ($draft['nilaiJasa'] ?? 0),
                "nomorAju"            => $nomorAju,
                "seri"                => (int) ($draft['seri'] ?? 0),
                "tanggalAju"          => $tanggalAju,
                "tanggalMasuk"        => !empty($draft['tanggalMasuk']) ? strval($draft['tanggalMasuk']) : $tanggalAju,
                ...
                "bankDevisa"          => $payloadBankDevisa,
                "pungutan"            => $payloadPungutan,
                "kesiapanBarang"      => $kesiapanBarangList, // Ditambahkan dari single
                "barang"              => $arrayBarang,
            ];

            // Filter field tanggal yang kosong
            foreach (['tanggalMasukPlb', 'tanggalMasuk', 'tanggalMuat', 'tanggalEkspor', 'tanggalPeriksa', 'tanggalTtd'] as $dateField) {
                if (isset($payload[$dateField]) && empty($payload[$dateField])) {
                    unset($payload[$dateField]);
                }
            }

            Log::info('Kirim Batch BC 3.3 CEISA Payload: ', $payload);
            $responseCeisa = $this->ceisaService->kirimDokumenBatch33($payload);
            ...
            if ($responseCeisa['successful']) {
                ...
                return response()->json([
                    'status'         => 200,
                    'message'        => 'Dokumen Batch BC 3.3 berhasil dikirim ke CEISA!',
                ...
                return response()->json([
                    'status'      => $responseCeisa['status_code'],
                    'message'     => 'Gagal mengirim Batch BC 3.3 ke CEISA.',
                    'ceisa_error' => $responseCeisa['body'],
                ], $responseCeisa['status_code']);
            }

        } catch (\Exception $e) {
            Log::error('Error Send CEISA Batch BC 3.3: ' . $e->getMessage(), ['trace' => $e->getTraceAsString()]);
            return response()->json([
                'status'  => 500,
