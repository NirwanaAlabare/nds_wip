...
class Bc262Service
{
    ...
    public function edit($id, Request $request)
    {
        ...
        $listValuta = \App\Services\BcReferenceService::getValuta();
                $listTujuanPengiriman = \App\Services\BcReferenceService::getTujuanPemasukan('262');
        ...
        $listKategoriBarang = \App\Services\BcReferenceService::getKategoriBarang('262');
        ...
        return view('export-import.dokumen-pabean.edit-bc262', [
            'header'              => $header,
            'items'               => $items,
            'ceisaInfo'           => $ceisaInfo,
    ...
    public function updateDraft($id, Request $request)
    {
        ...
        try {
            ...
            return response()->json([
                'status'  => 200,
                'message' => 'Draft BC 2.6.2 berhasil disimpan.'
            ]);

        } catch (\Exception $e) {
            DB::connection('mysql_sb')->rollBack();
            Log::error('Error Update Draft BC 2.6.2: ' . $e->getMessage(), ['trace' => $e->getTraceAsString()]);
            return response()->json([
                'status'  => 500,
        ...
        try {
            ...
            foreach (($draft['barang'] ?? []) as $index => $b) {
                ...
                $payloadBarang[] = [
                    ...
                    'kodeBarang'        => $b['kodeBarang'] ?? '',
                    'kodeDokumen'       => $b['kodeDokumen'] ?? '', // Default BC 2.0 atau BC lainnya
                    'kodeJenisKemasan'  => $b['kodeJenisKemasan'] ?? '',
            ...
            $finalPayload = [
                ...
                'kodeTujuanPemasukan' => $draft['tujuanPengiriman'] ?? '',
            ...
            Log::info('Kirim BC 2.6.2 CEISA Payload: ', $finalPayload);
            ...
            if ($responseCeisa['successful']) {
                ...
                return response()->json([
                    'status'         => 200,
                    'message'        => 'Dokumen BC 2.6.2 berhasil dikirim ke CEISA!',
                ...
                return response()->json([
                    ...
                    'message'     => 'Gagal mengirim BC 2.6.2 ke CEISA.',
                    'ceisa_error' => $responseCeisa['body']
                ], $responseCeisa['status_code']);
            }

        } catch (\Exception $e) {
            Log::error('Error Send CEISA BC 2.6.2: ' . $e->getMessage(), ['trace' => $e->getTraceAsString()]);
            return response()->json([
                'status'  => 500,
                'message' => $e->getMessage()
            ], 500);
    ...
    private function generateNomorAju($db)
    {
        ...
        $ceisaSeq = $this->ceisaService->getLastSequenceFromCeisa($prefix . $currentYear, '262');
    ...
    public function editBatch($ids, Request $request) {
        ...
        return view('export-import.dokumen-pabean.edit-batch-bc262', [
            'header'              => $header,
            'items'               => $items,
            'ceisaInfo'           => $ceisaInfo,
            'dataDetail'          => $dataDetail,
    ...
    public function updateDraftBatchBc262($ids, Request $request)
    {
        $db = DB::connection('mysql_sb');
        $db->beginTransaction();
        ...
        try {
            ...
            foreach ($bpbs as $id) {
                ...
                $bpbno_int = $header->bpbno_int ?? null;
    ...
    public function sendCeisaBatch262(array $bpbs, Request $request)
    {
        $db = DB::connection('mysql_sb');
        ...
        try {
            $draftRow = $db->table('bpb_ceisa')->where('bpbno', $firstBpb)->orWhere('bpbno_int', $firstBpb)->first();
            if (!$draftRow || empty($draftRow->payload_json)) {
                throw new \Exception('Draft kosong. Simpan draft terlebih dahulu.');
            }
            ...
            $responseCeisa = $this->ceisaService->kirimDokumenBatch262($finalPayload, 'false');
            ...
            if ($responseCeisa['successful']) {
                foreach ($bpbs as $no_bpb) {
                    ...
                    if ($updated === 0) {
                        Log::warning("BC 2.6.2: bpbno {$no_bpb} tidak ditemukan di tabel bpb saat update nomor_aju.");
                    }
