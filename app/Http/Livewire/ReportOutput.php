    ...
    public function render()
    {
        ...
        if ($this->group == 'line') {
            $lines = UserLine::selectRaw("
                    ...
                    SUM(CASE WHEN master_plan.tgl_plan ".$masterPlanDateFilter1." THEN ( master_plan.man_power * master_plan.jam_kerja ) ELSE 0 END)*60 mins_avail,
                    MAX(CASE WHEN master_plan.tgl_plan ".$masterPlanDateFilter1." THEN ( master_plan.man_power ) ELSE 0 END) man_power,
                    MAX(CASE WHEN master_plan.tgl_plan ".$masterPlanDateFilter1." THEN ( master_plan.man_power ) ELSE 0 END)*(IF(cast(CURRENT_TIMESTAMP as time) <= '13:00:00', (FLOOR(TIME_TO_SEC(TIMEDIFF(cast(CURRENT_TIMESTAMP as time), '07:00:00'))/60)), ((FLOOR(TIME_TO_SEC(TIMEDIFF(cast(CURRENT_TIMESTAMP as time), '07:00:00'))/60))-60))) cumulative_mins_avail,
                    FLOOR(MAX(CASE WHEN master_plan.tgl_plan ".$masterPlanDateFilter1." THEN ( master_plan.man_power ) ELSE 0 END )*(IF(cast(CURRENT_TIMESTAMP as time) <= '13:00:00', (FLOOR(TIME_TO_SEC(TIMEDIFF(cast(CURRENT_TIMESTAMP as time), '07:00:00'))/60))/AVG(CASE WHEN master_plan.tgl_plan ".$masterPlanDateFilter1." THEN master_plan.smv ELSE 0 END), ((FLOOR(TIME_TO_SEC(TIMEDIFF(cast(CURRENT_TIMESTAMP as time), '07:00:00'))/60))-60)/AVG(CASE WHEN master_plan.tgl_plan ".$masterPlanDateFilter1." THEN master_plan.smv ELSE 0 END) ))) cumulative_target,
                    SUM(CASE WHEN master_plan.tgl_plan ".$masterPlanDateFilter1." THEN ( master_plan.plan_target ) ELSE 0 END) total_target,
                    userpassword.FullName,
                    userpassword.username,
                    SUM(CASE WHEN master_plan.tgl_plan ".$masterPlanDateFilter1." THEN ( master_plan.jam_kerja ) ELSE 0 END) jam_kerja,
                    MAX(CASE WHEN master_plan.tgl_plan ".$masterPlanDateFilter1." THEN ( DATE(master_plan.tgl_plan) ) ELSE 0 END) tgl_plan,
                    GREATEST(IFNULL(MAX(CASE WHEN master_plan.tgl_plan ".$masterPlanDateFilter1." THEN ( rfts.last_rft ) ELSE 0 END), MAX(CASE WHEN master_plan.tgl_plan ".$masterPlanDateFilter1." THEN ( master_plan.tgl_plan ) ELSE 0 END)), IFNULL(MAX(CASE WHEN master_plan.tgl_plan ".$masterPlanDateFilter1." THEN ( defects.last_defect ) ELSE 0 END), MAX(CASE WHEN master_plan.tgl_plan ".$masterPlanDateFilter1." THEN ( master_plan.tgl_plan ) ELSE 0 END)), IFNULL(MAX(CASE WHEN master_plan.tgl_plan ".$masterPlanDateFilter1." THEN ( reworks.last_rework ) ELSE 0 END), MAX(CASE WHEN master_plan.tgl_plan ".$masterPlanDateFilter1." THEN ( master_plan.tgl_plan ) ELSE 0 END)), IFNULL(MAX(CASE WHEN master_plan.tgl_plan ".$masterPlanDateFilter1." THEN ( rejects.last_reject ) ELSE 0 END), MAX(CASE WHEN master_plan.tgl_plan ".$masterPlanDateFilter1." THEN ( master_plan.tgl_plan ) ELSE 0 END)) ) latest_output")->
                leftJoin("master_plan", "userpassword.username", "=", "master_plan.sewing_line")->
                leftJoin("act_costing", "act_costing.id", "=", "master_plan.id_ws")->
        ...
        if ($this->group == 'ws' || $this->group == 'style') {
            if ($this->group == 'ws') {
                ...
                groupBy("act_costing.kpno", "master_plan.id_ws", "act_costing.styleno", "master_plan.sewing_line")->
                havingRaw("tgl_plan ".$masterPlanDateFilter." OR (tgl_plan ".$masterPlanDateFilter1." AND SUM((IFNULL(rfts.rft, 0)+IFNULL(reworks.rework, 0))) > 0)")->
                get();
                ...
                groupBy("act_costing.styleno", "master_plan.sewing_line")->
                havingRaw("tgl_plan ".$masterPlanDateFilter." OR (tgl_plan ".$masterPlanDateFilter1." AND SUM((IFNULL(rfts.rft, 0)+IFNULL(reworks.rework, 0))) > 0)")->
                get();
            }
        } else {
        ...
        $defectTypes = DB::connection("mysql_sb")->table('output_defects'.$this->qcType)->
            ...
            where("master_plan.cancel", 'N')->
            whereRaw("DATE(output_defects".$this->qcType.".updated_at) ".$masterPlanDateFilter." AND master_plan.tgl_plan ".$masterPlanDateFilter1."")->
