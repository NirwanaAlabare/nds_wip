...
where created_at >= '$tgl_filter' and created_at <= '$tgl_filter'
group by created_by, so_det_id
) a
left join master_sb_ws m on a.so_det_id = m.id_so_det
left join master_size_new msn on m.size = msn.size
left join (select id_so_det, group_concat(distinct(po)) list_po from ppic_master_so where tgl_shipment >= '$tgl_filter' and tgl_shipment <= '$tgl_shipment' group by id_so_det) b on a.so_det_id = b.id_so_det
