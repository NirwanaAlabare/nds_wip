                            ...
                            FROM
                                ...
                                LEFT JOIN (
                                    SELECT
                                        ...
                                        MAX(secondary_inhouse_in_input.qty_in) as qty_akhir,
                                        MAX(secondary_inhouse_in_input.urutan) AS max_urutan,
                                    ...
                                    LEFT JOIN stocker_input ON stocker_input.id_qr_stocker = secondary_inhouse_in_input.id_qr_stocker
                                    LEFT JOIN part_detail_secondary ON part_detail_secondary.part_detail_id = stocker_input.part_detail_id and part_detail_secondary.urutan = secondary_inhouse_in_input.urutan
                                    LEFT JOIN master_secondary ON master_secondary.id = part_detail_secondary.master_secondary_id
                                    WHERE secondary_inhouse_in_input.created_at >= '".date("Y-m-d", strtotime($dateFrom." -1 year"))."'
                                    GROUP BY id_qr_stocker
                                    having MAX(secondary_inhouse_in_input.urutan) is not null
                                ) mx ON sii_in.id_qr_stocker = mx.id_qr_stocker AND sii_in.urutan = mx.max_urutan
                            WHERE
                                sii_in.tgl_trans > COALESCE((select MAX(tanggal) from dc_rekap), '2026-01-01') AND
                                sii_in.tgl_trans < '".$dateFrom."' AND
                                (
                                    sii_in.urutan IS NULL
                                    OR sii_in.urutan = mx.max_urutan
                                ) AND
                                s.id is not null AND
                                (s.cancel IS NULL OR s.cancel != 'y') and
                                (s.notes IS NULL OR s.notes NOT LIKE '%STOCKER MANUAL%') and
                        ...
                        sii as (
                            ...
                            FROM
                                ...
                                LEFT JOIN (
                                    ...
                                    LEFT JOIN master_secondary ON master_secondary.id = part_detail_secondary.master_secondary_id
                                    WHERE secondary_inhouse_input.created_at >= '".date("Y-m-d", strtotime($dateFrom." -1 year"))."'
                                    GROUP BY id_qr_stocker
                        ...
                        si as (
                            ...
                            FROM
                                ...
                                LEFT JOIN (
                                    ...
                                    LEFT JOIN master_secondary ON master_secondary.id = part_detail_secondary.master_secondary_id
                                    WHERE secondary_in_input.created_at >= '".date("Y-m-d", strtotime($dateFrom." -1 year"))."'
                                    GROUP BY id_qr_stocker
                                    having MAX(secondary_in_input.urutan) is not null
                                ) mx ON si.id_qr_stocker = mx.id_qr_stocker AND si.urutan = mx.max_urutan
                            WHERE
                                si.tgl_trans > COALESCE((select MAX(tanggal) from dc_rekap), '2026-01-01') AND
                                si.tgl_trans < '".$dateFrom."' AND
                                (
                                    si.urutan IS NULL
                                    OR si.urutan = mx.max_urutan
                                ) AND
                                s.id is not null AND
                                (s.cancel IS NULL OR s.cancel != 'y') and
                                (s.notes IS NULL OR s.notes NOT LIKE '%STOCKER MANUAL%') and
