...
class ReportHourlyController extends Controller
{
    public function index(Request $request)
    {
        ...
        $tgl_filter_prev = date('Y-m-d', strtotime($tgl_filter . ' -1 day'));
...
(
    ...
    order by a.tgl_trans asc, REPLACE(ul.username, '_', ' ') asc, sd.styleno_prod asc
            ...
            $data_tracking = DB::connection('mysql_sb')->select("WITH a as (
select
date(updated_at)tgl_trans,
so_det_id,
master_plan_id,
...
from output_rfts a
left join dim_jam_kerja_sewing b on time(a.updated_at) >= b.jam_kerja_awal and time(a.updated_at) <= b.jam_kerja_akhir
where updated_at >= '$tgl_filter 00:00:00' and updated_at <= '$tgl_filter 23:59:59'
group by master_plan_id, created_by, date(updated_at)
),
op as (
select date(output_rfts.updated_at) tgl_trans_line,max(time(output_rfts.updated_at)) jam_akhir_input_line,count(so_det_id) tot_output_line,
...
from output_rfts
left join user_sb_wip u on output_rfts.created_by = u.id
left join userpassword ul on ul.line_id = u.line_id
where output_rfts.updated_at >= '$tgl_filter 00:00:00' and output_rfts.updated_at <= '$tgl_filter 23:59:59'
group by REPLACE(ul.username, '_', ' '), date(output_rfts.updated_at)
),
acm as (
select * from act_costing_mfg where id_item = '8' group by id_act_cost
),
...
(
		...
		created_by
		from output_rfts a
		where updated_at >= '$tgl_filter 00:00:00' and updated_at <= '$tgl_filter 23:59:59' and status = 'NORMAL'
		group by master_plan_id, created_by, date(updated_at)
		) a
		inner join master_plan mp on a.master_plan_id = mp.id
		group by tgl_trans_rfts, master_plan_id
...
cmp as (
select min(id), man_power, sewing_line, tgl_plan from master_plan where date(tgl_plan) >= '$tgl_filter 00:00:00' and  date(tgl_plan) <= '$tgl_filter 23:59:59' and cancel = 'N' group by sewing_line, tgl_plan
),
e_skrg as (
		select
		sewing_line,
		...
		left join user_sb_wip on output_rfts.created_by = user_sb_wip.id
		left join userpassword on user_sb_wip.line_id = userpassword.line_id
		where output_rfts.updated_at >= '$tgl_filter 00:00:00' and output_rfts.updated_at <= '$tgl_filter 23:59:59' group by REPLACE(userpassword.username, '_', ' '), date(output_rfts.updated_at)
		) op on z.tgl_trans = op.tgl_trans_line and REPLACE(ul.username, '_', ' ') = op.username
		left join
		(
		select min(id), man_power, sewing_line, tgl_plan from master_plan where date(tgl_plan) >= '$tgl_filter 00:00:00' and  date(tgl_plan) <= '$tgl_filter 23:59:59' and cancel = 'N' group by sewing_line, tgl_plan
...
jk as (
	...
	left join userpassword ul on ul.line_id = u.line_id
	where a.updated_at >= '$tgl_filter 00:00:00' and a.updated_at <= '$tgl_filter 23:59:59' and status = 'NORMAL'
...
inner join master_plan mp on a.master_plan_id = mp.id
inner join mastersupplier ms on ac.id_buyer = ms.Id_Supplier
left join op on a.tgl_trans = op.tgl_trans_line and REPLACE(ul.username, '_', ' ') = op.username
left join acm on ac.id = acm.id_act_cost
left join d_rfts on a.tgl_trans = d_rfts.tgl_trans_rfts and a.master_plan_id = d_rfts.master_plan_id
left join cmp on a.tgl_trans = cmp.tgl_plan and ul.username = cmp.sewing_line
left join e_skrg on REPLACE(ul.username, '_', ' ') = e_skrg.sewing_line
left join jk on REPLACE(ul.username, '_', ' ') = jk.username and sd.styleno_prod = jk.styleno_prod
left join td on REPLACE(ul.username, '_', ' ') = td.sewing_line and sd.styleno_prod = td.styleno
...
order by a.tgl_trans asc, REPLACE(ul.username, '_', ' ') asc, sd.styleno_prod asc
            ...
            $tot_eff = $total_mins_avail > 0
                ? round(($total_mins_prod / $total_mins_avail) * 100, 2)
                : 0;

            $tot_eff_percent = $tot_eff . ' %';
