...
public function ReportRekonsiliasi(Request $request)
{
    if ($request->ajax()) {
        ...
        $data_ceisa = DB::connection('mysql_sb')->select("select a.*, COALESCE(b.keterangan,'-') keterangan_update from (select *, CASE
            ...
            ) a GROUP BY bcno, jenis_dok) b on b.nomor_aju = a.no_aju and b.bcno = a.no_daftar and b.jenis_dok = a.kode_dokumen) a where a.tgl_daftar >= '".$request->dateFrom."' and a.tgl_daftar <= '".$request->dateTo."' ".$additionalQuery.") a LEFT JOIN
