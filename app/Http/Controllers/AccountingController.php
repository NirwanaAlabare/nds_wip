...
public function ReportRekonsiliasi(Request $request)
{
    if ($request->ajax()) {
        ...
        END AS status_kesesuaian from  (
            select kode_dokumen, no_aju, tgl_aju, a.no_daftar, a.tgl_daftar, ROUND(qty,2) qty, ROUND(total,2) total, ROUND(total_idr,2) total_idr, ROUND(COALESCE(qty_sb,0),2) qty_sb, ROUND(COALESCE(total_sb,0),2) total_sb, ROUND(COALESCE(total_sb * IF(c.rate is null,'1',c.rate),0),2) total_sb_idr, ROUND(ROUND(qty,2) - ROUND(COALESCE(qty_sb,0),2),2) diff_qty, ROUND(ROUND(total_idr,2) - ROUND(COALESCE(total_sb,0) * IF(c.rate is null,'1',c.rate),2),2) diff_total, if(no_bpb is null,'-',no_bpb) no_bpb, IF(jenis_dok is null,'Not Updated','Updated') status, satuan_sb, satuan_sb_total, satuan_ciesa, satuan_ciesa_tampil, satuan_ciesa_total, nama_entitas, supplier, curr_ceisa, IF(c.rate is null,'1',c.rate) rate from (select * from (SELECT CONCAT('BC ',GROUP_CONCAT(SUBSTRING(kode_dokumen,n,1) ORDER BY n SEPARATOR '.')) AS kode_dokumen,nomor_aju,no_aju,tgl_aju,no_daftar,tgl_daftar,qty,satuan_ciesa, satuan_ciesa_tampil, satuan_ciesa_total,total,total_idr, nama_entitas, curr_ceisa FROM (
            select kode_dokumen,nomor_aju,no_aju,tgl_aju,no_daftar,tgl_daftar,ROUND(SUM(qty),2) AS qty,satuan_sb kode_satuan,ROUND(SUM(total),2) AS total,ROUND(SUM(total_idr),2) AS total_idr, GROUP_CONCAT(DISTINCT satuan_sb SEPARATOR ', ') AS satuan_ciesa, GROUP_CONCAT(DISTINCT kode_satuan SEPARATOR ', ') AS satuan_ciesa_tampil, GROUP_CONCAT(CONCAT(kode_satuan, ' (', round(qty,2), ')') SEPARATOR ', ') satuan_ciesa_total, nama_entitas, curr_ceisa from (select a.*, b.satuan_sb from (select a.*, b.nama_entitas from (SELECT kode_dokumen,nomor_aju,no_aju,tgl_aju,no_daftar,tgl_daftar,ROUND(SUM(jumlah_satuan),2) AS qty,kode_satuan,ROUND(SUM(cif),2) AS total,ROUND(SUM(cif_rupiah),2) AS total_idr,curr_ceisa FROM (
            SELECT a.*,kode_barang,uraian,jumlah_satuan,kode_satuan,cif,cif_rupiah FROM (
            SELECT kode_dokumen,nomor_aju,SUBSTRING(nomor_aju,-6) AS no_aju,DATE_FORMAT(STR_TO_DATE(SUBSTRING(nomor_aju,13,8),'%Y%m%d'),'%Y-%m-%d') AS tgl_aju,LPAD(nomor_daftar,6,0) AS no_daftar,tanggal_daftar AS tgl_daftar, if(kode_valuta = '','IDR', kode_valuta) curr_ceisa FROM exim_header where tanggal_daftar >= '".$request->dateFrom."' and tanggal_daftar <= '".$request->dateTo."'
            ) a LEFT JOIN (
        ...
        ) a GROUP BY bcno, jenis_dok) b on b.nomor_aju = a.no_aju and b.bcno = a.no_daftar and b.jenis_dok = a.kode_dokumen
				LEFT JOIN (select tanggal, curr, rate from masterrate where v_codecurr = 'PAJAK' GROUP BY tanggal, curr ) c on c.curr = a.curr_ceisa and c.tanggal = a.tgl_daftar) a where a.tgl_daftar >= '".$request->dateFrom."' and a.tgl_daftar <= '".$request->dateTo."' ".$additionalQuery.") a LEFT JOIN
