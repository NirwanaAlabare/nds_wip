...
class PPIC_LaporanTrackingController extends Controller
{
    public function index(Request $request)
    {
        ...
        $data_ws = DB::select("select ws isi , concat(buyer, ' - ', ws, ' - ', styleno) tampil
        from master_sb_ws where tgl_kirim >= '2024-01-01'
        group by ws");
    ...
    public function show_lap_tracking_ppic(Request $request)
    {
        ...
        delete from ppic_laporan_tracking_tmp_qc_output where created_by = '$user' and ws = '$ws'");

        $delete_tmp_p_line =  DB::delete("
        delete from ppic_laporan_tracking_tmp_packing_line where created_by = '$user' and ws = '$ws'");

        $data_qc = DB::connection('mysql_sb')->select("
select ms.supplier buyer, ac.kpno ws, sd.color, sd.size, sum(a.tot) tot_qc from
...
where ac.kpno = '$ws'
group by ac.kpno, sd.color, sd.size
order by ac.kpno asc, sd.color asc, msn.urutan asc
        ...
        $data_packing_line = DB::select("
        select buyer, ws, color, m.size, tot_p_line from
        ...
        union
                ...
                where ws = '$ws' and created_by = '$user'
        union
                select
                buyer,
                ws,
        ...
        order by color asc, urutan asc, a.size asc
                    ");
