...
class DefectSpotcleaningOutReportController extends Controller
{
    private function getDefectOutData($startDate, $endDate, $buyer)
    {
        ...
        // Kueri diekstrak dari Mutasi WIP (Fokus Defect Spotcleaning OUT / input_rework_spotcleaning)
        $sql = "
            WITH
            saldo_sewing_defect AS (
                SELECT so_det_id, SUM(input_rework_spotcleaning) AS input_rework_spotcleaning
                FROM (
                    SELECT so_det_id, SUM(CASE WHEN allocation = 'spotcleaning' AND defect_status IN ('REWORKED','REJECTED') THEN 1 ELSE 0 END) AS input_rework_spotcleaning
                    FROM signalbit_erp.output_defects a
                    INNER JOIN signalbit_erp.output_defect_types b ON a.defect_type_id = b.id
                    WHERE allocation = 'spotcleaning' AND (a.updated_at >= '$startDate 00:00:00') AND (a.updated_at <= '$endDate 23:59:59') AND defect_status IN ('REWORKED','REJECTED')
                    GROUP BY so_det_id
                ) defect GROUP BY so_det_id
            ),
            saldo_finishing_defect AS (
                SELECT so_det_id, SUM(input_rework_spotcleaning) AS input_rework_spotcleaning_f
                FROM (
                    SELECT so_det_id, SUM(CASE WHEN allocation = 'spotcleaning' AND defect_status IN ('REWORKED','REJECTED') THEN 1 ELSE 0 END) AS input_rework_spotcleaning
            ...
            mut AS (
                SELECT
                    ...
                    (COALESCE(ss.input_rework_spotcleaning, 0) + COALESCE(sf.input_rework_spotcleaning_f, 0)) AS total_out
                FROM (
    ...
    public function exportExcel(Request $request) {
        ...
        return Excel::download(new class($data, $start, $end) implements FromCollection, WithHeadings, WithStyles {
            ...
            public function headings(): array {
                return [ ["NIRWANA ALABARE GARMENT"], ["REPORT DEFECT SPOTCLEANING (OUT)"], ["Periode: $this->start s/d $this->end"], [""],
                    ["Buyer", "WS", "Style", "Color", "Size", "Jumlah Defect Spotcleaning OUT"] ];
            }
        ...
        }, 'Report_Defect_Spotcleaning_OUT_'.date('Ymd_His').'.xlsx');
    }
}
