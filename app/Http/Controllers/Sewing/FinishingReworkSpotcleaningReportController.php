...
class FinishingReworkSpotcleaningReportController extends Controller
{

    private function getReworkSpotData($startDate, $endDate, $buyer)
    {
        ...
        $sql = "
            ...
            UNION ALL
            select buyer, ws, styleno style, color, size, COALESCE(input_rework_spotcleaning_f,0) jumlah from signalbit_erp.inject_mutasi_sewing where type_saldo = 'FINISHING' AND input_rework_spotcleaning_f > 0 AND tgl_saldo >= '$startDate' AND tgl_saldo <= '$endDate'
    ...
    {
        $data = $this->getReworkSpotData($request->start_date, $request->end_date, $request->buyer);
    ...
    public function exportExcel(Request $request)
    {
        ...
        $data = collect($this->getReworkSpotData($start, $end, $request->buyer));

        $fileName = 'Report_Rework_Spotcleaning_Finishing_' . date('Ymd_His') . '.xlsx';
        ...
        return Excel::download(new class($data, $start, $end) implements FromCollection, WithHeadings, WithStyles {
            ...
            public function headings(): array {
                ...
                return [
                    ...
                    ["REPORT REWORK SPOTCLEANING (QC FINISHING)"],
                    [$periode],
                    [""],
                    ["Buyer", "WS", "Style", "Color", "Size", "Jumlah Rework Spotcleaning"]
                ];
            }
