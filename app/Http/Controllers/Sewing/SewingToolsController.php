...
class SewingToolsController extends Controller
{
    ...
    public function missMasterPlan(Request $request, SewingService $sewingService) {
        $kodeNumbering = $request->kode_numbering ? $request->kode_numbering : null;
        $updateOrigin = isset($request->update_origin) && $request->update_origin === 'true';

        return $sewingService->missMasterPlan($kodeNumbering, $updateOrigin);
    }
    ...
    // public function checkOutputDetailExportOld(Request $request) {
    //     $buyerFilterYs = "";
    //     $buyerFilterOutput = "";
    //     if ($request->buyer) {
    //         $buyerFilterYs = " and msb.buyer = '".$request->buyer."'";
    ...
    //     $back_date = $request->back_date;
    //     $back_date_packing = $request->back_date_packing;
    //     $tanggal_loading_awal = $request->tanggal_loading_awal;
    //     $tanggal_loading_akhir = $request->tanggal_loading_akhir;
    //     $tanggal_plan_awal = $request->tanggal_plan_awal;
    //     $tanggal_plan_akhir = $request->tanggal_plan_akhir;
    //     $tanggal_output_awal = $request->tanggal_output_awal;
    //     $tanggal_output_akhir = $request->tanggal_output_akhir;
    //     $tanggal_packing_awal = $request->tanggal_packing_awal;
    //     $tanggal_packing_akhir = $request->tanggal_packing_akhir;

    //     return Excel::download(new CheckOutputDetailListExport($buyer, $ws, $style, $color, $size, $kode, $tanggal_loading_awal, $tanggal_loading_akhir, $line_loading, $tanggal_plan_awal, $tanggal_plan_akhir, $tanggal_output_awal, $tanggal_output_akhir, $tanggal_packing_awal, $tanggal_packing_akhir, $line_output, $status_output, $defect_output, $allocation_output, $line_packing, $status_packing, $defect_packing, $allocation_packing, $crossline_loading, $crossline_output, $missmatch_code, $missmatch_code_packing, $back_date, $back_date_packing), 'Laporan Output Detail.xlsx');
    // }
    ...
    public function restoreUndoSubmit(Request $request)
    {
        ...
        if ($kodeNumbering) {
            if ($department == "packing_po") {
                ...
                $restoreData = UndoPacking::selectRaw("*, output_undo_packing.id as undo_id")->leftJoin("master_plan", "master_plan.id", "=", "output_undo_packing.master_plan_id")->whereRaw("output_undo_packing.kode_numbering is not null and output_undo_packing.kode_numbering in (".$kodeNumbering.")")->groupBy("output_undo_packing.kode_numbering")->get();
            } else {
                $restoreData = Undo::selectRaw("*, output_undo.id as undo_id")->leftJoin("master_plan", "master_plan.id", "=", "output_undo.master_plan_id")->whereRaw("output_undo.kode_numbering is not null and output_undo.kode_numbering in (".$kodeNumbering.")")->groupBy("output_undo.kode_numbering")->get();
            }
            $rft = [];
            $rftNds = [];
            $defect = [];
    ...
    public function injectAdjustmentSewing() {
        return view("sewing.tools.inject-adjustment-sewing", ['page' => 'dashboard-sewing-eff']);
    }

    public function contohUploadImportInjectAdjustmentSewing()
    {
        $path = public_path('assets/example/contoh-import-inject-adjustment-sewing.xlsx');
        return response()->download($path);
    }

    public function importDataInjectAdjustmentSewing(Request $request)
    {
        $file = $request->file('file');
    ...
    public function storeInjectAdjustmentSewing(Request $request)
    {
    ...
    public function deleteInjectAdjustmentSewing(Request $request)
    {
    ...
    public function getDataInjectAdjustmentSewing(Request $request)
    {
        $tglAwal = $request->dateFrom;
        $tglAkhir = $request->dateTo;
        $source = $request->source;
