...
class FinishingSpotcleaningReportController extends Controller
{
    private function getSpotData($startDate, $endDate, $buyer)
    {
        ...
        // Query Defect Spotcleaning (Allocation spotcleaning)
        $sql = "
            SELECT
            ...
            UNION ALL
            select buyer, ws, styleno style, color, size, COALESCE(defect_spotcleaning_f,0) jumlah from signalbit_erp.inject_mutasi_sewing where type_saldo = 'FINISHING' AND defect_spotcleaning_f > 0 AND tgl_saldo >= '$startDate' AND tgl_saldo <= '$endDate'
    ...
    {
        $data = $this->getSpotData($request->start_date, $request->end_date, $request->buyer);
    ...
    public function exportExcel(Request $request)
    {
        ...
        $data = collect($this->getSpotData($start, $end, $request->buyer));

        $fileName = 'Report_Defect_Spotcleaning_Finishing_' . date('Ymd_His') . '.xlsx';
        ...
        return Excel::download(new class($data, $start, $end) implements FromCollection, WithHeadings, WithStyles {
            ...
            public function headings(): array {
                ...
                return [
                    ...
                    ["REPORT DEFECT SPOTCLEANING (QC FINISHING)"],
                    [$periode],
                    [""],
                    ["Buyer", "WS", "Style", "Color", "Size", "Jumlah Defect Spotcleaning"]
                ];
            }
