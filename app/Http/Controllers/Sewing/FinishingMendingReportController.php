...
class FinishingMendingReportController extends Controller
{
    private function getMendingData($startDate, $endDate, $buyer)
    {
        ...
        // Query Defect Mending (Allocation mending)
        $sql = "
            SELECT
            ...
            WHERE b.allocation = 'mending'
              AND a.created_at >= ?
              AND a.created_at <= ?
              $buyerFilter
            GROUP BY ms.supplier, ac.kpno, ac.styleno, sd.color, sd.size, msn.urutan
            UNION ALL
            select buyer, ws, styleno style, color, size, COALESCE(defect_mending_f,0) jumlah from signalbit_erp.inject_mutasi_sewing where type_saldo = 'FINISHING' AND defect_mending_f > 0 AND tgl_saldo >= '$startDate' AND tgl_saldo <= '$endDate'
    ...
    {
        $data = $this->getMendingData($request->start_date, $request->end_date, $request->buyer);
    ...
    public function exportExcel(Request $request)
    {
        ...
        $data = collect($this->getMendingData($start, $end, $request->buyer));

        $fileName = 'Report_Defect_Mending_Finishing_' . date('Ymd_His') . '.xlsx';
        ...
        return Excel::download(new class($data, $start, $end) implements FromCollection, WithHeadings, WithStyles {
            ...
            public function headings(): array {
                ...
                return [
                    ...
                    ["REPORT DEFECT MENDING (QC FINISHING)"],
                    [$periode],
                    [""],
                    ["Buyer", "WS", "Style", "Color", "Size", "Jumlah Defect Mending"]
                ];
            }
