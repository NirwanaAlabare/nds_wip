...
class ReportMutasiOutputController extends Controller
{
    ...
    public function show_mut_output(Request $request)
    {
        ...
        if ($request->ajax()) {
            $data_mut = DB::connection('mysql_sb')->select("
            WITH m as (
                SELECT
                ms.supplier buyer,
                ac.kpno ws,
                ...
                saldo_packing_defect_awal as(
                    SELECT
                        defect.so_det_id,
                        SUM(input_rework_sewing) input_rework_pck_sewing,
                        SUM(input_rework_spotcleaning) input_rework_pck_spotcleaning,
                        SUM(input_rework_mending) input_rework_pck_mending,
                        SUM(defect_sewing) defect_pck_sewing,
                        SUM(defect_spotcleaning) defect_pck_spotcleaning,
                        SUM(defect_mending) defect_pck_mending
                    FROM (
                        SELECT
                            so_det_id,
                            SUM(CASE WHEN allocation = 'SEWING' THEN 1 ELSE 0 END) AS defect_sewing,
