...
class DefectMendingInReportController extends Controller
{
    public function index()
    {
        ...
        $list_tipe = [
            'defect_in'  => 'Defect Mending IN',
            'defect_out' => 'Defect Mending OUT'
        ];

        return view('sewing.report.report-defect-mending-inout-sentral', [
            'page'           => 'dashboard-sewing-eff',
            'subPageGroup'   => 'sewing-report',
            'subPage'        => 'reportDefectMendingInOut',
    ...
    private function getDefectInData($startDate, $endDate, $buyer)
    {
        ...
        $sql = "
            ...
            saldo_sewing_defect AS (
                SELECT so_det_id, SUM(defect_mending) AS defect_mending
                FROM (
                    SELECT so_det_id, SUM(CASE WHEN allocation = 'mending' THEN 1 ELSE 0 END) AS defect_mending
                    FROM signalbit_erp.output_defects a
                    INNER JOIN signalbit_erp.output_defect_types b ON a.defect_type_id = b.id
                    WHERE allocation = 'mending' AND (a.created_at >= '$startDate 00:00:00') AND (a.created_at <= '$endDate 23:59:59')
                    GROUP BY so_det_id
                ) defect GROUP BY so_det_id
            ),
            saldo_finishing_defect AS (
                SELECT so_det_id, SUM(defect_mending) AS defect_mending_f
                FROM (
                    SELECT so_det_id, SUM(CASE WHEN allocation = 'mending' THEN 1 ELSE 0 END) AS defect_mending
            ...
            mut AS (
                SELECT
                    ...
                    (COALESCE(ss.defect_mending, 0) + COALESCE(sf.defect_mending_f, 0)) AS total_in
                FROM (
    ...
    public function exportExcel(Request $request) {
        ...
        return Excel::download(new class($data, $start, $end) implements FromCollection, WithHeadings, WithStyles {
            ...
            public function headings(): array {
                return [ ["NIRWANA ALABARE GARMENT"], ["REPORT DEFECT MENDING (IN)"], ["Periode: $this->start s/d $this->end"], [""],
                    ["Buyer", "WS", "Style", "Color", "Size", "Jumlah Defect Mending IN"] ];
            }
        ...
        }, 'Report_Defect_Mending_IN_'.date('Ymd_His').'.xlsx');
    }
}
