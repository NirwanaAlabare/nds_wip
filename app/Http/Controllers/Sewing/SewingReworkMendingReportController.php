...
class SewingReworkMendingReportController extends Controller
{
    private function getReworkMendingData($startDate, $endDate, $buyer)
    {
        ...
        $sql = "
            SELECT
                ...
                COUNT(a.id) AS qty_rework_mending
    ...
    {
        return response()->json(['data' => $this->getReworkMendingData($request->start_date, $request->end_date, $request->buyer)]);
    }
    ...
    public function exportExcel(Request $request)
    {
        ...
        $data = collect($this->getReworkMendingData($start, $end, $request->buyer));
        ...
        return Excel::download(new class($data, $start, $end) implements FromCollection, WithHeadings, WithStyles {
            ...
            public function headings(): array {
                ...
                return [
                    ["NIRWANA ALABARE GARMENT"], ["REPORT REWORK MENDING"], [$periode], [""],
                    ["Buyer", "WS", "Style", "Color", "Size", "Jumlah Rework Mending"]
                ];
            }
        ...
        }, 'Report_Rework_Mending_'.date('Ymd_His').'.xlsx');
    }
}
