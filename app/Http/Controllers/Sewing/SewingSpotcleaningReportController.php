...
class SewingSpotcleaningReportController extends Controller
{
    public function index()
    {
        ...
        return view('sewing.report.report-spotcleaning-sewing', [
            'page'           => 'dashboard-sewing-eff',
            'subPageGroup'   => 'sewing-report',
            'subPage'        => 'reportSpotcleaning',
            'containerFluid' => true,
            'list_buyer'     => $list_buyer
        ]);
    }

    private function getSpotcleaningData($startDate, $endDate, $buyer)
    {
        ...
        $sql = "
            SELECT
                ...
                sd.color,
                sd.size,
                COUNT(a.id) AS qty_spotcleaning
    ...
    {
        $data = $this->getSpotcleaningData($request->start_date, $request->end_date, $request->buyer);
    ...
    public function exportExcel(Request $request)
    {
        ...
        $data = collect($this->getSpotcleaningData($start_date, $end_date, $request->buyer));

        $fileName = 'Report_Defect_Spotcleaning_' . date('Ymd_His') . '.xlsx';
