...
class PPIC_MonitoringMaterialSumController extends Controller
{
    public function ppic_monitoring_material_sum(Request $request)
    {
...
");
        ...
        return view(
            'ppic.monitoring_material_sum',
            [
                'page' => 'dashboard-ppic',
                "subPageGroup" => "ppic-monitoring",
                "subPage" => "ppic_monitoring_material_sum",
                "containerFluid" => true,
    ...
    public function get_ppic_monitoring_material_sum_style(Request $request)
    {
    ...
    public function show_lap_monitoring_material_f_sum(Request $request)
    {
        $user = Auth::user()->name;
...
CASE
    WHEN ABS(SUM(qty_pembelian)
             + SUM(qty_retur_subkon)
             + SUM(qty_retur_prod)
             + SUM(qty_adj)
             - SUM(qty_out_prod)
             - SUM(qty_out_subkon)
             - SUM(qty_retur_pembelian)
             - SUM(qty_out_sample)
             - SUM(qty_out_lainnya)
             - SUM(qty_out_adj)) < 0.005 THEN 0.00
    ELSE ROUND(
        SUM(qty_pembelian)
        + SUM(qty_retur_subkon)
        + SUM(qty_retur_prod)
