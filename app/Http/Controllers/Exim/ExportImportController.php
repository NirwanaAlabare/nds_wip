    ...
    public function ReportRekonsiliasi(Request $request)
    {
        if ($request->ajax()) {
            ...
            END AS status_kesesuaian from  (
                ...
                SELECT kode_dokumen,nomor_aju,SUBSTRING(nomor_aju,-6) AS no_aju,DATE_FORMAT(STR_TO_DATE(SUBSTRING(nomor_aju,13,8),'%Y%m%d'),'%Y-%m-%d') AS tgl_aju,LPAD(nomor_daftar,6,0) AS no_daftar,tanggal_daftar AS tgl_daftar FROM exim_header where tanggal_daftar BETWEEN '".$request->dateFrom."' and '".$request->dateTo."'
                ) a LEFT JOIN (
...
GROUP BY b.nomor_aju
        ...
        return view("export-import.rekonsiliasi-cesia.laporan-rekonsiliasi", ['jenisdok' => $jenisdok, 'statusdok' => $statusdok, "page" => "dashboard-export-import"]);
    }
...
GROUP BY b.nomor_aju
        ...
        return view("export-import.ceisa-detail.laporan-ceisa-detail", ['jenisdok' => $jenisdok, 'statusdok' => $statusdok, "page" => "dashboard-export-import"]);
    }
    ...
    public function ReportSignalbitBC(Request $request)
    {
        ...
        return view("export-import.data-bc-signalbit.report-signalbit-bc", ["page" => "dashboard-export-import"]);
    }
