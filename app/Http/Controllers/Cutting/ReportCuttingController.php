...
use App\Exports\Cutting\export_excel_report_mut_wip_cutting_detail;
use App\Exports\Cutting\export_excel_report_pengeluaran_cutting_panel;
...
class ReportCuttingController extends Controller
{
    public function buildOutputCuttingVsManagementRollQuery($dateFrom = null, $dateTo = null)
    {
        ...
        if ($dateFrom) {
            $additionalQuery .= " and a.waktu_selesai >= '" . $dateFrom . " 00:00:00'";
            $additionalQuery1 .= " and form_cut_piping.updated_at >= '" . $dateFrom . " 00:00:00'";
            $additionalQuery2 .= " and form_cut_piece.waktu_selesai >= '" . $dateFrom . " 00:00:00'";
        }

        if ($dateTo) {
            $additionalQuery .= " and a.waktu_selesai <= '" . $dateTo . " 23:59:59'";
            $additionalQuery1 .= " and form_cut_piping.updated_at <= '" . $dateTo . " 23:59:59'";
            $additionalQuery2 .= " and form_cut_piece.waktu_selesai <= '" . $dateTo . " 23:59:59'";
        }
        ...
        $query = "
            ...
            FROM (
                ...
                select
                    ...
                    no_form_cut_input no_form,
                    act_costing_ws worksheet,
                    style,
                    color,
                    panel,
                    0 qty_output,
                ...
                FROM (
                    select * from (
                        ...
                        select
                            ...
                            a.unit_l_act unit_lebar_actual,
                            COALESCE(b.id_roll, '-') id_roll,
                            COALESCE(b.id_item, a.notes_new, a.notes) id_item,
                            COALESCE(b.detail_item, a.notes_new, a.notes) detail_item,
                            COALESCE(b.roll_buyer, b.roll) roll,
                            COALESCE(b.lot, '-') lot,
                            COALESCE(b.group_roll, '-') group_roll,
                            (
     ...
     */
    public function outputCuttingVsManagementRoll(Request $request)
    {
        if ($request->ajax()) {
            $dateFrom = ($request->dateFrom ? $request->dateFrom : date("Y-m-d"));
            $dateTo = ($request->dateTo ? $request->dateTo : date("Y-m-d"));

            $data = DB::select($this->buildOutputCuttingVsManagementRollQuery($dateFrom, $dateTo));

            return DataTables::of($data)->toJson();
        }

        return view('cutting.report.output-cutting-vs-management-roll', [
            'page' => 'dashboard-cutting',
            'subPageGroup' => 'cutting-report',
            'subPage' => 'output-cutting-vs-management-roll',
        ]);
    }

    public function exportOutputCuttingVsManagementRoll(Request $request)
    {
        $dateFrom = ($request->dateFrom ? $request->dateFrom : date("Y-m-d"));
        $dateTo = ($request->dateTo ? $request->dateTo : date("Y-m-d"));
        ...
        $fileName = 'output-cutting-vs-management-roll';
        ...
        $sheet->writeRow(
            ['Output Cutting vs Management Roll'],
            [
                'font-style' => 'bold',
                'font-size'  => 14,
            ]
    ...
    public function pemakaianRoll(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $pemakaianRoll = DB::connection("mysql_sb")->select("
                ...
                (select a.no_dok, GROUP_CONCAT(DISTINCT no_invoice) no_req,id_item,COUNT(no_barcode) total_roll_ri, sum(qty_sj) qty_out_ri,satuan from (select * from whs_inmaterial_fabric where no_dok like '%RI%' and supplier = 'Production - Cutting' ) a INNER JOIN (select bppbno,bppbdate from bppb_req where bppbno like '%RQ-F%' and id_supplier = '432' and bppbdate between '" . $dateFrom . "' and '" . $dateTo . "' GROUP BY bppbno) b on b.bppbno = a.no_invoice INNER JOIN whs_lokasi_inmaterial c on c.no_dok = a.no_dok GROUP BY no_invoice,id_item) c on c.no_req = a.bppbno and c.id_item  =a.id_item
            ");
...
order by  ws asc, color asc
    ...
    public function buildMutasiFabricProporsionalQuery($startDate, $endDate, $type)
    {
        ...
        $query = "
            ...
            FROM (
                ...
                SELECT
                    ...
                    COALESCE(
                    (
                        SELECT SUM(retur.qty_aktual)
                        FROM retur
                        WHERE retur.no_ws = wip_adjustment_fabric.ws
                        AND retur.no_barcode = wip_adjustment_fabric.id_roll
                        AND retur.id_item = wip_adjustment_fabric.id_item
                        AND retur.satuan = wip_adjustment_fabric.satuan
                    ), 0) qty_retur_adjustment,
                    0 saldo,
    ...
    public function report_cutting_mutasi_fabric_proporsional(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            if ($start_date === null || $end_date === null) {
                ...
                $mutasiFabriPropQuery = $this->buildMutasiFabricProporsionalQuery($start_date, $end_date, $tipe);

                $rawData = DB::select($mutasiFabriPropQuery, [$start_date, $end_date, $prev_date]);
    ...
    public function export_excel_report_cutting_mutasi_fabric_proporsional(Request $request)
    {
        ...
        $data = DB::select($mutasiFabriPropQuery, [$start_date, $end_date, $prev_date]);
...
order by tanggal asc, no_form asc
    ...
    // {
    //     return Excel::download(new F($request->start_date, $request->end_date), 'Laporan_Penerimaan FG_Stok.xlsx');
    // }
    ...
    public function report_mutasi_wip_cutting1(Request $request)
    {
        ...
        // For non-AJAX (initial page load)
        return view(
            'cutting.report.report_mutasi_wip_cutting',
            [
                'page' => 'dashboard-cutting',
                "subPageGroup" => "cutting-report",
                "subPage" => "report_mutasi_wip_cutting",
                'containerFluid' => true
            ]
        );
    }

    public function report_mutasi_wip_cutting(Request $request)
    {
                ...
                $rawData = DB::select("WITH
                    cutt_awal as (
                        ...
                        FROM (
                            ...
                            WHERE
                                    form_cut_input.`status` = 'SELESAI PENGERJAAN' and
                                    COALESCE(DATE(form_cut_input.waktu_selesai), DATE(form_cut_input.waktu_mulai), DATE(form_cut_input.tgl_input)) >= '$tgl_saldo' and
                                    COALESCE(DATE(form_cut_input.waktu_selesai), DATE(form_cut_input.waktu_mulai), DATE(form_cut_input.tgl_input)) < '$start_date' and
                                    (marker_input_detail.ratio > 0 OR (similar.max_group = form_cut_input_detail.group_stocker AND modify_size_qty.difference_qty > 0))
                            GROUP BY
                                    form_cut_input.id,
                                    form_cut_input_detail.group_stocker,
                    ...
                    cutt_saldo_awal as (
                        SELECT
                            id_so_det, panel, SUM(qty_cut_awal) - SUM(qty_dc_awal) + SUM(qty_cut) saldo_awal
                        FROM (
                            SELECT id_so_det, panel, sum(saldo) qty_cut_awal, 0 as qty_dc_awal, 0 AS qty_cut, 0 AS qty_dc, 0 as qty_replace FROM mut_cut_pcs_tmp where tgl_trans = '$tgl_saldo' group by id_so_det, panel
                            UNION ALL
                            SELECT id_so_det, panel, 0 qty_cut_awal, 0 as qty_dc_awal, sum(qty) AS qty_cut, 0 AS qty_dc, 0 as qty_replace FROM cutt_awal group by id_so_det, panel
                            UNION ALL
                            SELECT id_so_det, panel, 0 AS qty_cut_awal, sum(qty_dc) AS qty_dc_awal, 0 as qty_cut, 0 as qty_dc, 0 as qty_replace FROM dc_awal group by id_so_det, panel
                        ) saldo_awal
                        group by
                            id_so_det,
                            panel
                    ...
                    FROM
                    (
                        select id_so_det, panel, sum(saldo_awal) qty_cut_awal, 0 as qty_dc_awal, 0 AS qty_cut, 0 AS qty_dc, 0 as qty_replace from cutt_saldo_awal group by id_so_det, panel
                        UNION ALL
                        SELECT id_so_det, panel, 0 AS qty_cut_awal, 0 AS qty_dc_awal, sum(qty) qty_cut, 0 AS qty_dc, 0 as qty_replace FROM cutt_in group by id_so_det, panel
                        UNION ALL
    ...
    // Deprecated
        ...
        //     // For non-AJAX (initial page load)
        //     return view(
        //         'cutting.report.report_mutasi_wip_cutting_detail',
        //         [
        //             'page' => 'dashboard-cutting',
        //             "subPageGroup" => "cutting-report",
        //             "subPage" => "report_mutasi_wip_cutting_detail",
        //             'containerFluid' => true
        //         ]
        //     );
        // }

    public function buildReportMutasiWipCuttingDetailQuery($start_date, $end_date, $tgl_saldo)
    {
            ...
            $query = "
                ...
                query_cutting as (
                    WITH
                        cutt_awal as (
                                ...
                                UNION ALL
                                        ...
                                        FROM
                                                ...
                                                LEFT JOIN part p_com ON p_com.id = pd_com.part_id
                                                LEFT JOIN master_part ON master_part.id = part_detail.master_part_id
                                                LEFT JOIN part_custom pcust ON pcust.part_id = part.id and pcust.part_detail_id = part_detail.id and pcust.color = form_cut_piece.color
                                        WHERE
                                                COALESCE(DATE(form_cut_piece.waktu_selesai), DATE(form_cut_piece.created_at), DATE(form_cut_piece.updated_at)) >= '".$tgl_saldo."'
                                                AND COALESCE(DATE(form_cut_piece.waktu_selesai), DATE(form_cut_piece.created_at), DATE(form_cut_piece.updated_at)) < '".$start_date."'
                                                AND form_cut_piece_detail.STATUS = 'complete'
                                ...
                                UNION ALL
                                        ...
                                        FROM
                                            ...
                                            LEFT JOIN part p_com ON p_com.id = pd_com.part_id
                                            LEFT JOIN master_part ON master_part.id = part_detail.master_part_id
                                            LEFT JOIN part_custom pcust ON pcust.part_id = part.id and pcust.part_detail_id = part_detail.id and pcust.color = form_cut_reject.color
                                        WHERE
                                            form_cut_reject.tanggal >= '".$tgl_saldo."'
                                            AND form_cut_reject.tanggal < '".$start_date."'
                                            AND form_cut_reject.tanggal > '2026-04-30'
                        ...
                        saldo_awal_cutting as (
                                ...
                                left join part_detail on part_detail.id = mut_cut_pcs_tmp_detail.part_detail_id
                                left join master_part on master_part.id = part_detail.master_part_id
                                where mut_cut_pcs_tmp_detail.tgl_trans = '2026-05-01'
                                group by
                                        master_sb_ws.id_so_det,
                                        part.id,
                                        part.panel,
                ...
                form_list as (
                    ...
                    from
                        ...
                        left join part_detail on part_detail.part_id = part.id
                        left join master_part mp on mp.id = part_detail.master_part_id
                        left join part_custom pcust on pcust.part_id = part.id and pcust.part_detail_id = part_detail.id and pcust.color = query_cutting.color
                    where
                        part.panel_status != 'COMPLEMENT' and COALESCE(pcust.set_part_status, part_detail.part_status) != 'COMPLEMENT'
                    group by
                        part.id,
                        part_detail.id
                )
    ...
    public function report_mutasi_wip_cutting_detail(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            if ($start_date === null || $end_date === null) {
                ...
                $query = $this->buildReportMutasiWipCuttingDetailQuery($start_date, $end_date, $tgl_saldo);
    ...
    // public function export_excel_report_mut_wip_cutting(Request $request)
    // {
    //     return Excel::download(new export_excel_report_mut_wip_cutting($request->start_date, $request->end_date), 'Laporan Mutasi WIP Cutting.xlsx');
    // }
    ...
    public function report_pengeluaran_cutting(Request $request)
    {
        ...
        // For non-AJAX (initial page load)
        return view(
            'cutting.report.report_pengeluaran_cutting',
            [
                'page' => 'dashboard-cutting',
                "subPageGroup" => "cutting-report",
                "subPage" => "report_pengeluaran_cutting",
                'containerFluid' => true
            ]
        );
    }

    public function report_pengeluaran_cutting_panel(Request $request)
    {
        ...
        // For non-AJAX (initial page load)
        return view(
            'cutting.report.report_pengeluaran_cutting_panel',
            [
                'page' => 'dashboard-cutting',
                "subPageGroup" => "cutting-report",
                "subPage" => "report_pengeluaran_cutting_panel",
                'containerFluid' => true
            ]
        );
    }
                ...
                $rawData = DB::connection('mysql_sb')->select("
                    ...
                    WHERE
                        ...
                        AND DATE(whs_inmaterial_fabric.tgl_dok)
                            BETWEEN ? AND ?
                        AND (
                            whs_lokasi_inmaterial.no_dok <> 'GK/RI/0526/00159'
                            OR (
                                whs_lokasi_inmaterial.no_dok = 'GK/RI/0526/00159'
                                AND whs_lokasi_inmaterial.no_barcode IN (
                                    'F260415608','F260415619','F260415620','F260415621','F260415622','F260415623','F260415624','F260415625','F260415626','F260415627','F260415628',
                                    'F260415629','F260415630','F260415631','F260415632','F260415633','F260415634','F260415635','F260415636','F260415637','F260415638','F260415639',
                                    'F260415640','F260415641','F260415642','F260415643','F260415644','F260415645','F260415646','F260415647','F260415648','F260415649','F260415650',
                                    'F260415651','F260415652','F260415653','F260415654','F260417791','F260417792','F260417793','F260417794','F260417795','F260417796','F260417797',
                                    'F260417798','F260417799','F260417800','F260417801','F260417802','F260417803','F260417804','F260417811','F260417816','F260418249','F260418250',
                                    'F260418251','F260414142','F260418252','F260418253','F260418254','F260418255','F260418256','F260418257','F260418258','F260418259','F260418260',
                                    'F260418261','F260418262','F260418263','F260418264','F260418265','F260418266','F260418267','F260418268','F260418269','F260418270','F260418271',
                                    'F260418272','F260418273','F260418274','F260418275','F260418276','F260418277','F260418278','F260418279','F260418280','F260418281','F260418282',
                                    'F260418283','F260418284','F260418285','F260418286','F260418287','F260418288','F260418289','F260418290','F260414181','F260418291','F260418292',
                                    'F260418293','F260418294','F260418295','F260418296','F260418297','F260418298','F260418299','F260418300','F260418301','F260418302','F260418303',
                                    'F260418304','F260418305','F260418306','F260418307','F260418308','F260415547','F260415554','F260415576','F260415582'
                                )
                            )
                        )
                ...
                $rawData = DB::select("WITH
                    ...
                    qty_in_panel AS (
                        SELECT
                            ...
                            p.qty_before,
                            p.qty
                        FROM qty_in_per_panel p
                        INNER JOIN bom b
                            ON b.ws = p.worksheet
                            AND b.color = p.color
                            AND b.nama_panel = p.panel
                    ),
                    ...
                    qty_out_per_panel AS (
                        SELECT
                            act_costing_ws AS worksheet,
                            buyer,
                            style,
                            color,
                            size,
