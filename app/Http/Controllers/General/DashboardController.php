    ...
    // Sewing
        public function sewingEff(Request $request) {
            ...
            if ($request->ajax()) {
                ...
                $sewingEfficiencyData = DB::connection('mysql_sb')->table('master_plan')->
                    ...
                    ")->
                    leftJoin(DB::raw("(SELECT count(rfts.id) rft, master_plan.id master_plan_id, DATE(rfts.updated_at) tgl_output from output_rfts rfts inner join master_plan on master_plan.id = rfts.master_plan_id where (rfts.updated_at between '".date('Y-m-d', strtotime("$year-$month-01"))." 00:00:00' and '".date('Y-m-t', strtotime($year."-".$month."-01 00:00:00"))." 23:59:59') and status = 'NORMAL' and (master_plan.tgl_plan between '".date('Y-m-d', strtotime("$year-$month-01 - 10 days"))."' and '".date('Y-m-t', strtotime($year."-".$month."-01"))."') GROUP BY master_plan.id, master_plan.tgl_plan, DATE(rfts.updated_at)) as rfts"), function ($join) { $join->on("master_plan.id", "=", "rfts.master_plan_id"); $join->on("master_plan.tgl_plan", "=", "rfts.tgl_output"); })->
                    leftJoin(DB::raw("(SELECT count(defects.id) defect, master_plan.id master_plan_id, DATE(defects.updated_at) tgl_output from output_defects defects inner join master_plan on master_plan.id = defects.master_plan_id where defects.defect_status = 'defect' and (defects.updated_at between '".date('Y-m-d', strtotime("$year-$month-01"))." 00:00:00' and '".date('Y-m-t', strtotime($year."-".$month."-01 00:00:00"))." 23:59:59') and (master_plan.tgl_plan between '".date('Y-m-d', strtotime("$year-$month-01 - 10 days"))."' and '".date('Y-m-t', strtotime($year."-".$month."-01"))."') GROUP BY master_plan.id, master_plan.tgl_plan, DATE(defects.updated_at)) as defects"), function ($join) { $join->on("master_plan.id", "=", "defects.master_plan_id"); $join->on("master_plan.tgl_plan", "=", "defects.tgl_output"); })->
                    leftJoin(DB::raw("(SELECT count(defrew.id) rework, master_plan.id master_plan_id, DATE(defrew.updated_at) tgl_output from output_defects defrew inner join master_plan on master_plan.id = defrew.master_plan_id where defrew.defect_status = 'reworked' and (defrew.updated_at between '".date('Y-m-d', strtotime("$year-$month-01"))." 00:00:00' and '".date('Y-m-t', strtotime($year."-".$month."-01 00:00:00"))." 23:59:59') and (master_plan.tgl_plan between '".date('Y-m-d', strtotime("$year-$month-01 - 10 days"))."' and '".date('Y-m-t', strtotime($year."-".$month."-01"))."') GROUP BY master_plan.id, master_plan.tgl_plan, DATE(defrew.updated_at)) as reworks"), function ($join) { $join->on("master_plan.id", "=", "reworks.master_plan_id"); $join->on("master_plan.tgl_plan", "=", "reworks.tgl_output"); })->
                    leftJoin(DB::raw("(SELECT count(rejects.id) reject, master_plan.id master_plan_id, DATE(rejects.updated_at) tgl_output from output_rejects rejects inner join master_plan on master_plan.id = rejects.master_plan_id where (rejects.updated_at between '".date('Y-m-d', strtotime("$year-$month-01"))." 00:00:00' and '".date('Y-m-t', strtotime($year."-".$month."-01 00:00:00"))." 23:59:59') and (master_plan.tgl_plan between '".date('Y-m-d', strtotime("$year-$month-01 - 10 days"))."' and '".date('Y-m-t', strtotime($year."-".$month."-01"))."') GROUP BY master_plan.id, master_plan.tgl_plan, DATE(rejects.updated_at)) as rejects"), function ($join) { $join->on("master_plan.id", "=", "rejects.master_plan_id"); $join->on("master_plan.tgl_plan", "=", "rejects.tgl_output"); })->
                    leftJoin(DB::raw("(
                        SELECT
                            tgl_output,
                            SUM(rft) rft,
                        ...
                        FROM
                            (
                                ...
                                where
                                    ...
                                    AND master_plan.tgl_plan >= DATE_SUB('".$year."-".$month."-01', INTERVAL 30 DAY) AND master_plan.tgl_plan <= '".$year."-".$month."-31'
                                GROUP BY
        ...
        public function sewingSummary(Request $request) {
            ...
            $sewingSummaryData = DB::connection('mysql_sb')->table('master_plan')->
                ...
                leftJoin(DB::raw("(SELECT count(defrew.id) rework, master_plan.id master_plan_id from output_defects defrew inner join master_plan on master_plan.id = defrew.master_plan_id where defrew.defect_status = 'reworked' and ( defrew.updated_at between '".date('Y-m-d', strtotime("$year-$month-01"))." 00:00:00' and '".date('Y-m-t', strtotime($year."-".$month."-01 00:00:00"))." 23:59:59' ) and (master_plan.tgl_plan between '".date('Y-m-d', strtotime("$year-$month-01 - 10 days"))."' and '".date('Y-m-t', strtotime($year."-".$month."-01"))."') GROUP BY master_plan.id, master_plan.tgl_plan) as reworks"), "master_plan.id", "=", "reworks.master_plan_id")->
                leftJoin(DB::raw("(SELECT count(rejects.id) reject, master_plan.id master_plan_id from output_rejects rejects inner join master_plan on master_plan.id = rejects.master_plan_id where ( rejects.updated_at between '".date('Y-m-d', strtotime("$year-$month-01"))." 00:00:00' and '".date('Y-m-t', strtotime($year."-".$month."-01 00:00:00"))." 23:59:59' ) and (master_plan.tgl_plan between '".date('Y-m-d', strtotime("$year-$month-01 - 10 days"))."' and '".date('Y-m-t', strtotime($year."-".$month."-01"))."') GROUP BY master_plan.id, master_plan.tgl_plan) as rejects"), "master_plan.id", "=", "rejects.master_plan_id")->
                where("master_plan.cancel", 'N')->
                whereRaw("(
                    MONTH(master_plan.tgl_plan) = '".$month."'
                    AND
        ...
        public function sewingOutputData(Request $request) {
            $dateFrom = $request->date_from ? $request->date_from : date('Y-m-01');
            $dateTo   = $request->date_to   ? $request->date_to   : date('Y-m-t');
