        ...
        if ($request->ajax()) {
            ...
            if ($start_date === null || $end_date === null) {
                ...
                if ($start_date < $today && $end_date < $today) {
                    ...
                    blc_earn_and_cost,
                    tot_man_power,
                    est_earn_per_min_all_man_power,
                    est_cost_per_min_all_man_power,
                    blc_earn_and_cost_all_man_power,
                    kurs_tengah,
                    est_earning_rate,
                    est_tot_cost_rate,
                    blc_est_earning_cost_rate,
                    est_earning_rate_per_min,
                    est_cost_rate_per_min,
                    blc_rate_per_min,
                    est_earning_rate_per_man_power,
                    est_cost_rate_per_man_power,
                    blc_rate_per_man_power
                    FROM mgt_rep_tmp_sum_prod_earning WHERE tanggal >= '$start_date' and tanggal <= '$end_date'");
                } else {
...
sum_prod_earn as (
        ...
        coalesce(sewing_absen_menit,0) as sewing_absen_menit,
        ROUND(coalesce(b.sum_tot_earning_rupiah,0) / coalesce(sewing_absen_menit,0),2) as est_earn_per_min,
        ROUND(coalesce(sum_tot_labor,0) / coalesce(sewing_absen_menit,0),2) as est_cost_per_min,
        ROUND((coalesce(b.sum_tot_earning_rupiah,0) / coalesce(sewing_absen_menit,0)) - (coalesce(sum_tot_labor,0) / coalesce(sewing_absen_menit,0)),2) as blc_earn_and_cost,
        coalesce(d.tot_man_power,0) as tot_man_power,
        ROUND(coalesce(b.sum_tot_earning_rupiah,0) / coalesce(d.tot_man_power,0),2) as est_earn_per_min_all_man_power,
        ROUND(coalesce(sum_tot_labor,0) / coalesce(d.tot_man_power,0),2) as est_cost_per_min_all_man_power,
        ROUND((coalesce(b.sum_tot_earning_rupiah,0) / coalesce(d.tot_man_power,0)) - (coalesce(sum_tot_labor,0) / coalesce(d.tot_man_power,0)),2) blc_earn_and_cost_all_man_power,

        e.kurs_tengah,
        ROUND(coalesce(b.sum_tot_earning_rupiah,0) / coalesce(e.kurs_tengah,0),2) as est_earning_rate,
        ROUND(coalesce(sum_tot_labor,0) / coalesce(e.kurs_tengah,0),2) as est_tot_cost_rate,
        ROUND((coalesce(b.sum_tot_earning_rupiah,0) / coalesce(e.kurs_tengah,0)) - (coalesce(sum_tot_labor,0) / coalesce(e.kurs_tengah,0)),2) as blc_est_earning_cost_rate,

        ROUND((coalesce(b.sum_tot_earning_rupiah,0) / coalesce(e.kurs_tengah,0)) / coalesce(sewing_absen_menit,0),2) as est_earning_rate_per_min,
        ROUND(((coalesce(sum_tot_labor,0) / coalesce(e.kurs_tengah,0)) / coalesce(sewing_absen_menit,0)),2) as est_cost_rate_per_min,
        ROUND(((coalesce(b.sum_tot_earning_rupiah,0) / coalesce(e.kurs_tengah,0)) / coalesce(sewing_absen_menit,0)) - ((coalesce(sum_tot_labor,0) / coalesce(e.kurs_tengah,0)) / coalesce(sewing_absen_menit,0)),2) as blc_rate_per_min,

        ROUND((coalesce(b.sum_tot_earning_rupiah,0) / coalesce(e.kurs_tengah,0)) /  coalesce(d.tot_man_power,0),2) as est_earning_rate_per_man_power,
        ROUND(((coalesce(sum_tot_labor,0) / coalesce(e.kurs_tengah,0)) / coalesce(d.tot_man_power,0)),2) as est_cost_rate_per_man_power,
        ROUND(((coalesce(b.sum_tot_earning_rupiah,0) / coalesce(e.kurs_tengah,0)) / coalesce(d.tot_man_power,0)) - ((coalesce(sum_tot_labor,0) / coalesce(e.kurs_tengah,0)) / coalesce(d.tot_man_power,0)),2) as blc_rate_per_man_power
...
select * from sum_prod_earn where tanggal >= '$start_date' and tanggal <= '$end_date'
order by tanggal asc
    ...
    {
        return Excel::download(new export_excel_laporan_sum_prod_earn($request->start_date, $request->end_date), 'Laporan_Penerimaan FG_Stok.xlsx');
    }
}
