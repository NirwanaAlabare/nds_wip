    ...
    public function dashboard_packing(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $data_input = DB::select("SELECT
                ...
                from
                (
                select created_by_line created_by,so_det_id,count(so_det_id) qty_p_line from signalbit_erp.output_rfts_packing_po
    ...
    public function show_tot_dash_packing(Request $request)
    {
        $data_header = DB::select("
            ...
            (
                ...
                from output_rfts_packing where created_at >='$request->dateFilter 00:00:00' and created_at <= '$request->dateFilter 23:59:59'
                union
                select '0' tot_p_line,sum(qty) tot_trf_garment ,'0' tot_packing_in , '0' tot_packing_out
                from packing_trf_garment where created_at >='$request->dateFilter 00:00:00' and created_at <= '$request->dateFilter 23:59:59'
                union
                select '0' tot_p_line,'0' tot_trf_garment,sum(qty) tot_packing_in, '0' tot_packing_out
                from packing_packing_in where created_at >='$request->dateFilter 00:00:00' and created_at <= '$request->dateFilter 23:59:59'
                union
                select '0' tot_p_line,'0' tot_trf_garment,'0' tot_packing_in,count(barcode) tot_packing_out
                from packing_packing_out_scan where created_at >='$request->dateFilter 00:00:00' and created_at <= '$request->dateFilter 23:59:59'
            ) a
        ");
