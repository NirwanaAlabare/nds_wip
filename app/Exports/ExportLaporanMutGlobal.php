...
(select id_item,unit, sum(sal_awal) sal_awal from (select 'SA' id,id_item,unit, sum(qty_good) sal_awal from whs_inmaterial_fabric_det where tgl_dok < '" . $this->from . "' and status = 'Y' GROUP BY id_item,unit union select 'TR' id,id_item,unit, round(sum(qty),2) sal_awal from whs_sa_fabric  where tgl_bpb < '" . $this->from . "' GROUP BY id_item,unit) a  GROUP BY id_item,unit) b on b.id_item = a.id_item and b.unit = a.unit left join
(select id_item,unit, sum(qty_good) qty_in from whs_inmaterial_fabric_det where tgl_dok BETWEEN '" . $this->from . "' and '" . $this->to . "' and status = 'Y' GROUP BY id_item,unit) c on c.id_item = a.id_item and c.unit = a.unit left join
...
(select id_item,satuan, sum(qty_out) qty_out from whs_bppb_det a inner join whs_bppb_h b on b.no_bppb = a.no_bppb where b.tgl_bppb BETWEEN '" . $this->from . "' and '" . $this->to . "' and a.status = 'Y' GROUP BY id_item,satuan) e on e.id_item = a.id_item and e.satuan = a.unit) a GROUP BY a.id_item,a.unit) a where fil != 0");
