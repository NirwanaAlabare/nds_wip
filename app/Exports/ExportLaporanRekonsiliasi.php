...
class ExportLaporanRekonsiliasi implements FromView, WithEvents, ShouldAutoSize
{
...

    {
        ...
        $data = DB::connection('mysql_sb')->select("select a.*, COALESCE(b.keterangan,'-') keterangan_update from (select *, CASE
            ...
            SELECT kode_dokumen,nomor_aju,SUBSTRING(nomor_aju,-6) AS no_aju,DATE_FORMAT(STR_TO_DATE(SUBSTRING(nomor_aju,13,8),'%Y%m%d'),'%Y-%m-%d') AS tgl_aju,LPAD(nomor_daftar,6,0) AS no_daftar,tanggal_daftar AS tgl_daftar FROM exim_header where tanggal_daftar BETWEEN '".$this->from."' and '".$this->to."'
            ) a LEFT JOIN (
...
GROUP BY b.nomor_aju
            ...
            ) a GROUP BY bcno, jenis_dok, nomor_aju) b on b.nomor_aju = a.no_aju and b.bcno = a.no_daftar and b.jenis_dok = a.kode_dokumen) a where a.tgl_daftar >= '".$this->from."' and a.tgl_daftar <= '".$this->to."' ".$additionalQuery.") a LEFT JOIN
