    ...
    public function view(): View
    {
        ...
        if ($this->start_date < $today && $this->end_date < $today) {
            $rawData = DB::connection('mysql_sb')->select("SELECT
                    ...
                    FROM mgt_rep_tmp_sum_prod_earning WHERE tanggal >= '$this->start_date' and tanggal <= '$this->end_date'");
        } else {
...
CASE
    ...
    ELSE
        ...
        2)
END AS est_tot_cost,
ROUND((COALESCE(tot_earning_rupiah, 0) - CASE
    WHEN stat_kerja = 'libur' THEN
        ROUND(COALESCE(sum_tot_labor, 0), 2)
    WHEN COALESCE(c.sum_mins_avail, 0) = 0
      OR COALESCE(a.mins_avail, 0) = 0 THEN
...
select * from sum_prod_earn where tanggal >= '$this->start_date' and tanggal <= '$this->end_date'
order by tanggal asc");
        }
