...
class OutputExport implements FromView, ShouldAutoSize
{
    ...
    public function view(): View
    {
        $lines = UserLine::selectRaw("
                COUNT(DISTINCT act_costing.kpno) total_ws,
                ...
                userpassword.FullName, userpassword.username,
            ...
            leftJoin(DB::raw("(SELECT max(defects.updated_at) last_defect, master_plan.id master_plan_id from output_defects".$this->subtype." defects inner join master_plan on master_plan.id = defects.master_plan_id where defects.defect_status = 'defect' and DATE(defects.updated_at) = '".$this->date."' GROUP BY master_plan.id, master_plan.tgl_plan) as defects"), "master_plan.id", "=", "defects.master_plan_id")->
            leftJoin(DB::raw("(SELECT max(defrew.updated_at) last_rework, count(defrew.id) rework, master_plan.id master_plan_id from output_defects".$this->subtype." defrew inner join master_plan on master_plan.id = defrew.master_plan_id where defrew.defect_status = 'reworked' and DATE(defrew.updated_at) = '".$this->date."' GROUP BY master_plan.id, master_plan.tgl_plan) as reworks"), "master_plan.id", "=", "reworks.master_plan_id")->
            leftJoin(DB::raw("(SELECT max(rejects.updated_at) last_reject, master_plan.id master_plan_id from output_rejects".$this->subtype." rejects inner join master_plan on master_plan.id = rejects.master_plan_id where DATE(rejects.updated_at) = '".$this->date."' GROUP BY master_plan.id, master_plan.tgl_plan) as rejects"), "master_plan.id", "=", "rejects.master_plan_id")->
            where("userpassword.Groupp", 'SEWING')->
            where("master_plan.cancel", 'N')->
            whereRaw("(master_plan.tgl_plan  = '".$this->date."' OR (IFNULL(rfts.rft, 0)+IFNULL(reworks.rework, 0)) > 0)")->
        ...
        $defectTypes = DB::connection('mysql_sb')->table('output_defects'.$this->subtype)->
            ...
            where("master_plan.cancel", 'N')->
            whereRaw("DATE(output_defects".$this->subtype.".updated_at) = '".$this->date."'")->
