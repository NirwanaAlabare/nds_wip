        ...
        $data_mut = DB::select("
                ...
                ORDER BY
                    ...
                    msn.urutan)
                                        ...
                                        UNION
                                        select msn.urutan, ws, color, styleno style, a.size, buyer, 0 pl_saldo_awal, COALESCE(packing_rft,0) pl_rft, 0 pl_reject, 0 pl_keluar, 0 pl_saldo_akhir, 0 pc_saldo_awal, 0 pc_terima, 0 pc_packing_scan, 0 pc_saldo_akhir from signalbit_erp.inject_mutasi_sewing a LEFT JOIN master_size_new msn ON msn.size = a.size  where type_saldo = 'FINISHING') a GROUP BY urutan, ws, color, style, size, buyer ORDER BY ws, color, buyer, urutan
            ");
