...
class Export_excel_rep_packing_mutasi implements FromView, WithEvents, ShouldAutoSize
{
    ...
    public function view(): View
    {
        ...
        $data_mut = DB::select("
                ...
                /* ================= PACKING LINE SALDO AWAL MASUK (HISTORIS) ================= */
                    SELECT
                        so_det_id,
                        COUNT(*) AS pl_saldo_awal_masuk,
                        0 AS pl_saldo_awal_keluar,
                ...
                /* ================= PACKING LINE SALDO AWAL KELUAR (HISTORIS TRANSFER GARMENT) ================= */
                    SELECT
                        pms.id_so_det AS so_det_id,
                        0 AS pl_saldo_awal_masuk,
                ...
                /* ================= PACKING CENTRAL SALDO AWAL MASUK (HISTORIS) ================= */
                    SELECT
                        pms.id_so_det AS so_det_id,
                        0 AS pl_saldo_awal_masuk,
                        0 AS pl_saldo_awal_keluar,
                ...
                /* ================= PACKING CENTRAL SALDO AWAL KELUAR (HISTORIS POS SCAN) ================= */
                    SELECT
                        id_so_det AS so_det_id,
                        0 AS pl_saldo_awal_masuk,
                        0 AS pl_saldo_awal_keluar,
                ...
                ORDER BY
                    ...
                    msn.urutan)
                                        ...
                                        select urutan, ws, color, style, a.size, buyer, sum(pl_saldo_awal) pl_saldo_awal, sum(pl_rft) pl_rft, sum(pl_reject) pl_reject, sum(pl_keluar) pl_keluar, sum(pl_saldo_akhir) pl_saldo_akhir, sum(pc_saldo_awal) pc_saldo_awal, sum(pc_terima) pc_terima, sum(pc_packing_scan) pc_packing_scan, sum(pc_saldo_akhir) pc_saldo_akhir from (select * from final_query
                                        UNION ALL
                                        select msn.urutan, ws, color, styleno style, a.size, buyer, 0 pl_saldo_awal, COALESCE(packing_rft,0) pl_rft, 0 pl_reject, 0 pl_keluar, 0 pl_saldo_akhir, 0 pc_saldo_awal, 0 pc_terima, 0 pc_packing_scan, 0 pc_saldo_akhir from signalbit_erp.inject_mutasi_sewing a LEFT JOIN master_size_new msn ON msn.size = a.size  where type_saldo = 'FINISHING' and tgl_saldo BETWEEN '{$tgl_awal} 00:00:00' AND '{$tgl_akhir} 23:59:59') a GROUP BY urutan, ws, color, style, size, buyer ORDER BY ws, color, buyer, urutan
            ");
    ...
    public static function afterSheet(AfterSheet $event)
    {
        ...
        $event->sheet->styleCells(
            'A3:N' . ($event->getConcernable()->rowCount + 2),
            [
                'borders' => [
                    'allBorders' => [
                        'borderStyle' => \PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN,
