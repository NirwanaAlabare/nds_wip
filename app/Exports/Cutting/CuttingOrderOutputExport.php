...
class CuttingOrderOutputExport implements FromView, WithEvents, ShouldAutoSize
{
    ...
    public function view(): View
    {
                ...
                FROM
                    ...
                    union
                        ...
                        WHERE
                            ...
                            AND COALESCE ( DATE ( form_cut_reject.updated_at ), DATE ( form_cut_reject.created_at ), form_cut_reject.tanggal ) >= '".$this->dateFrom."'
                            AND COALESCE ( DATE ( form_cut_reject.updated_at ), DATE ( form_cut_reject.created_at ), form_cut_reject.tanggal ) <= '".$this->dateTo."' AND form_cut_reject.tanggal >= DATE ( NOW()- INTERVAL 2 YEAR )
                            ".($this->order ? "AND form_cut_reject.act_costing_id = '".$this->order."'" : "")."
                        GROUP BY
                            form_cut_reject.id,
                            form_cut_reject_detail.so_det_id
                    union
        ...
        $dailyOrderOutputs = $dailyOrderOutputSql;

        // Date List
        $dates = $dailyOrderOutputs
            ->pluck('tanggal')
            ->unique()
            ->sort()
            ->values();
        ...
        $dailyOrderGroup = $dailyOrderOutputs
            ->groupBy(function ($item) use ($groupBy) {
                return implode('|', [
                    ...
                    $item->panel,
                    $item->id_meja,
                    $groupBy === 'size' ? $item->size : 'all',
                ]);
            })
            ->map(function ($items) {
                return $items->first(); // representative row
            })
            ->values();

        // Group Qty
        $outputSums = [];
        foreach ($dailyOrderOutputs as $row) {
            $key = implode('|', [
                $row->ws,
                $row->style,
                $row->color,
                $row->panel,
                $row->id_meja,
                $groupBy === 'size' ? $row->size : 'all',
                $row->tanggal,
            ]);

            if (!isset($outputSums[$key])) {
                $outputSums[$key] = 0;
            }

            $outputSums[$key] += $row->qty;
        }
        ...
        return view('cutting.export.cutting-order-output-export', [
            ...
            'dateTo' => $this->dateTo,
            'dates' => $dates,
            'dailyOrderGroup' => $dailyOrderGroup,
