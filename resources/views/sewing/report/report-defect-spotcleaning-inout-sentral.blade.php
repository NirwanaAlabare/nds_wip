...
<div class="card card-sb">
    <div class="card-header">
        <h5 class="card-title fw-bold mb-0"><i class="fas fa-search-minus"></i> Defect Spotcleaning (IN & OUT) Report</h5>
    </div>
    <div class="card-body">
...
<script>
    let table;
    const routeMap = {
        'defect_in': { data: "{{ route('report.defect_spotcleaning_in.data') }}", export: "{{ route('report.defect_spotcleaning_in.export') }}" },
        'defect_out': { data: "{{ route('report.defect_spotcleaning_out.data') }}", export: "{{ route('report.defect_spotcleaning_out.export') }}" }
    };
    ...
    function refreshTable() {
        ...
        $("#label_jumlah").text("Jumlah " + $("#filter_tipe option:selected").text().replace('Defect Spotcleaning ', ''));
        initDatatable(tipe);
    }
