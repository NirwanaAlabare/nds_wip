...
@section('custom-link')
    ...
    <style>
        ...
        .bg-light-spot { background-color: #f0f8ff !important; }
    </style>
@endsection
...
<div class="card card-sb">
    <div class="card-header">
        <h5 class="card-title fw-bold mb-0"><i class="fas fa-broom"></i> Report Defect Spotcleaning</h5>
    </div>
    <div class="card-body">
            ...
            <table id="datatable" class="table table-bordered table-sm table-custom table-hover w-100">
                <thead>
                    <tr>
                        ...
                        <th>Color</th>
                        <th>Size</th>
                        <th>Jumlah Defect Spotcleaning</th>
                    </tr>
                </thead>
                <tbody></tbody>
            </table>
...
<script>
    ...
    $(document).ready(function() {
        ...
        table = $('#datatable').DataTable({
            ...
            ajax: {
                url: "{{ route('report-sewing.spotcleaning.data') }}",
            ...
            columns: [
                ...
                { data: 'color' },
                { data: 'size' },
                { data: 'qty_spotcleaning', render: $.fn.dataTable.render.number(',', '.', 0), className: 'text-center fw-bold' }
            ]
        });
    });
    function refreshTable() { table.ajax.reload(null, false); }
    function exportExcel() {
        let start = $('#start_date').val(); let end = $('#end_date').val(); let buyer = $('#filter_buyer').val();
        if(!start || !end) { Swal.fire({ icon: 'warning', title: 'Perhatian', text: 'Tgl tidak boleh kosong!' }); return; }
        Swal.fire({ title: 'Menyiapkan Excel...', allowOutsideClick: false, didOpen: () => { Swal.showLoading(); }});
        window.location.href = `{{ route('report-sewing.spotcleaning.export') }}?start_date=${start}&end_date=${end}&buyer=${encodeURIComponent(buyer)}`;
