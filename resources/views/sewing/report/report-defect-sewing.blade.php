...
<div class="card card-sb">
    <div class="card-header">
        <h5 class="card-title fw-bold mb-0"><i class="fas fa-bug"></i> Report Defect Sewing</h5>
    </div>
    <div class="card-body">
            ...
            <table id="datatable" class="table table-bordered table-sm table-custom table-hover w-100">
                <thead>
                    <tr>
                        ...
                        <th width="10%">Size</th>
                        <th width="15%">Jumlah Defect Sewing</th>
                    </tr>
                </thead>
                <tbody>
                    </tbody>
...
<script>
    ...
    $(document).ready(function() {
        ...
        table = $('#datatable').DataTable({
            ...
            serverSide: false,
            ajax: {
                url: "{{ route('report-sewing.defect.data') }}",
                data: function(d) {
            ...
            columns: [
                { data: 'buyer', render: function(data) { return `<span class="text-center">${data}</span>`; }},
                { data: 'ws', render: function(data) { return `<span class="text-center">${data}</span>`; }},
                { data: 'style', render: function(data) { return `<span class="text-center">${data}</span>`; }},
                { data: 'color', render: function(data) { return `<span class="text-center">${data}</span>`; }},
                { data: 'size', render: function(data) { return `<span class="text-center">${data}</span>`; }},
                {
                    data: 'defect_sewing',
    ...
    function exportExcel() {
        ...
        let url = `{{ route('report-sewing.defect.export') }}?start_date=${start}&end_date=${end}&buyer=${encodeURIComponent(buyer)}`;
