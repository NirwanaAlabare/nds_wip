                ...
                <tr>
                    @if ($currentLine != $line->username)
                        <td rowspan="{{ $currentRowSpan }}" style="text-align: center; vertical-align: middle;">
                            <a style="text" href="{{ url('dashboard-wip/wip-line/' . $line->username) }}" target="_blank">
    ...
    <tbody>
        ...
        @else
            ...
            @foreach ($defectTypes as $type)
                ...
                @endphp
                <tr>
                    <td {{ $typeRowspan > 1 ? 'rowspan='.$typeRowspan : '' }} style="vertical-align: middle;">{{ $loop->iteration }}</td>
