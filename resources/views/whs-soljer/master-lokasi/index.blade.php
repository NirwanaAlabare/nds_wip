...
<div class="modal fade" id="modal-active-lokasi">
    <form action="{{ route('updatestatus-master-lokasi-whs-soljer') }}" method="post" onsubmit="submitForm(this, event)">
         @method('GET')
 ...
 <div class="modal fade" id="modal-edit-lokasi" tabindex="-1" aria-hidden="true">
  <form action="{{ route('simpan-edit-master-lokasi-whs-soljer') }}" method="post" onsubmit="submitForm(this, event)">
    @csrf
    @method('GET')
...
<div class="modal fade" id="modal-tambah-lokasi" tabindex="-1" aria-hidden="true">
  <form action="{{ route('store-lokasi-master-lokasi-whs-soljer') }}" method="post" onsubmit="submitForm(this, event)">
    @csrf
    @method('POST')
...
<script>
    let datatable = $("#datatable").DataTable({
        ...
        ajax: {
            ...
            url: '{{ route('master-lokasi-whs-soljer') }}',
            dataType: 'json',
            dataSrc: 'data',
            data: function(d) {
                d.area = $('#area').val();
...
function printAllLokasi() {
  ...
  $.ajax({
    url: '{{ route('print-lokasi-all-master-lokasi-whs-soljer') }}',
    type: 'POST',
    processData: false,
    contentType: false,
