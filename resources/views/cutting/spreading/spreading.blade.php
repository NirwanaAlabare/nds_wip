    ...
    <script>
            ...
            columnDefs: [
                {
                    ...
                    render: (data, type, row, meta) => {
                        ...
                        let btnDelete = row.status == 'SPREADING' ? "<a href='javascript:void(0);' data='"+encodeURIComponent(JSON.stringify(row))+"' data-url='{{ route('destroy-spreading') }}/"+row['id']+"' class='btn btn-danger btn-sm' data-bs-toggle='tooltip' data-bs-title='Hapus Form' onclick='deleteData(this);'><i class='fa fa-trash'></i></a>" : "<button class='btn btn-danger btn-sm' data-bs-toggle='tooltip' data-bs-title='Hapus Form' onclick='deleteData(this);' disabled><i class='fa fa-trash'></i></button>";
                        let btnPDF = "<a href='javascript:void(0);' class='btn btn-dark btn-sm' data='"+encodeURIComponent(JSON.stringify(row))+"' data-bs-toggle='tooltip' data-bs-title='Cetak Form' onclick='printForm(this);'><i class='fa fa-file-pdf'></i></a>";
        ...
        function printForm(element) {
            if (element.getAttribute('data')) {
                let data = JSON.parse(decodeURIComponent(element.getAttribute('data')));
