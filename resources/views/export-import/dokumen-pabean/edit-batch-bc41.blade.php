...
<style>
    ...
    /* Tambahan styling untuk penanda Batch */
    .badge-batch { font-size: 14px; padding: 5px 10px; background-color: #28a745; color: white; border-radius: 4px; }
</style>
@endsection
...
<div class="card card-sb">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h5 class="card-title fw-bold mb-0">
            <i class="fas fa-layer-group text-success"></i> BC 4.1 - EDIT DRAFT (BATCH)
        </h5>
        <span class="badge-batch"><i class="fas fa-check-double"></i> Mode Batch</span>
    </div>


    <form action="{{ route('dokumen-pabean-update_draft_batch_bc41', $batch_id ?? $header->trx_no_par) }}" method="POST" id="form-edit-ceisa" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="card-body">
            <div class="alert alert-success py-2 mb-4" style="background-color: #d4edda; border-color: #c3e6cb; color: #155724;">
                <div class="row align-items-center">
                    <div class="col-md-8">
                        <strong>ID Batch:</strong> {{ $batch_id ?? $header->trx_no_par }} <br>
                        <strong>Supplier / Vendor:</strong> {{ $header->supplier ?? '-' }}
                    </div>
                    <div class="col-md-4 text-right">
                        <small>Total Item Gabungan: <b>{{ count($items) }} Item</b></small>
                    </div>
                </div>
            </div>
            ...
            <div class="tab-content mt-3" id="ceisaTabContent">
                ...
                <div class="tab-pane fade" id="tab-dokumen" role="tabpanel">
                    <div class="card shadow-sm mb-3 border">
                        <div class="card-header text-white fw-bold d-flex align-items-center px-3 py-2" style="font-size:13px; background-color: #001f3f;">
                            <span>Dokumen Lampiran (Batch)</span>
                                        ...
                                        <td>
                                            <select name="pengangkut[{{ $sIndex }}][kodeCaraAngkut]" class="form-control form-control-sm select2bs4">
                                                ...
                                                @foreach($listCaraAngkut as $k => $v)
                                                    <option value="{{ $k }}" {{ ($sarkut['kodeCaraAngkut'] ?? '') == $k ? 'selected' : '' }}>{{ $k }} - {{ strtoupper($v) }}</option>
                                                @endforeach
                                            </select>
                                        </td>
                ...
                <div class="tab-pane fade" id="tab-transaksi" role="tabpanel">
                    <div class="row">
                        <div class="col-md-4">
                            <div class="card shadow-sm mb-3 border">
                                <div class="card-header fw-bold bg-light text-dark px-3 py-2" style="font-size:13px;">Nilai Transaksi (Total Batch)</div>
                                <div class="card-body">
                                    <div class="form-group mb-2">
                        ...
                        <div class="col-md-4">
                            <div class="card shadow-sm mb-3 border">
                                <div class="card-header fw-bold bg-light text-dark px-3 py-2" style="font-size:13px;">Volume & Berat (Total Batch)</div>
                                <div class="card-body">
                                    <div class="form-group mb-2">
                                    ...
                                    <div class="card-header py-2 btn-collapse-barang" data-target="#collapseBarang{{ $index }}" style="cursor: pointer; background-color: #001f3f;">
                                        <div class="d-flex justify-content-between align-items-center">
                                            <div class="fw-bold text-white" style="font-size: 13px;">
                                                {{ $item->goods_code ?? '' }} - {{ $item->itemdesc }} - ID ITEM : {{ $item->id_item ?? '' }}
                                            </div>
                                    ...
                                    <div id="collapseBarang{{ $index }}" class="collapse {{ $index == 0 ? 'show' : '' }}" data-parent="#accordionBarang">
                                        <div class="card-body py-3 px-3 bg-white">
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="col-md-4">
                                                    <div class="card shadow-sm mb-3 border">
                                                        <div class="card-header fw-bold bg-light text-dark px-3 py-2" style="font-size:13px;">Kuantitas & Nilai (Merge)</div>
                                                        <div class="card-body">
                                                            <div class="form-group mb-2">
        ...
        <div class="card-footer text-right bg-white border-top">
            ...
            <button type="submit" class="btn btn-success btn-sm"><i class="fas fa-save"></i> Simpan Draft Batch</button>
        </div>
    </form>
</div>
@endsection
...
<script>
    $(document).ready(function() {
        ...
        $('#form-edit-ceisa').on('submit', function(e) {
            ...
            let form = $(this);
            Swal.fire({
                title: 'Simpan Draft Batch?',
                text: "Data draft BC 4.1 (Batch) akan diperbarui.",
                icon: 'question',
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            Swal.fire({
                                title: 'Berhasil!',
                                text: response.message || 'Draft Batch BC 4.1 berhasil disimpan.',
                                icon: 'success'
                            }).then(() => {
