...
@section('custom-link')
    ...
    <style>
        .table-custom th { background-color: var(--sb-color) !important; color: white; white-space: nowrap; text-align: center; vertical-align: middle !important; font-size: 13px; }
        .table-custom td { vertical-align: middle !important; font-size: 13px; }
        .input-table { width: 100%; min-width: 80px; border: 1px solid #ced4da; border-radius: 4px; padding: 4px 8px; font-size: 13px; }
        .input-table:focus { outline: none; border-color: #80bdff; box-shadow: 0 0 0 0.2rem rgba(0,123,255,.25); }
        .input-table[readonly] { background-color: #e9ecef; pointer-events: none; }
        .select2-container--bootstrap4 .select2-selection--single { height: calc(1.5em + .5rem + 2px) !important; }
        .table-scroll-wrapper { max-height: 75vh; overflow-y: auto; }
        .table-scroll-wrapper thead th { position: sticky; top: 0; z-index: 10; background-color: var(--sb-color); box-shadow: 0 1px 2px rgba(0,0,0,0.1); }
    </style>
@endsection
...
<div class="card card-sb">
    ...
    <div class="card-header bg-sb">
        <div class="d-flex justify-content-between align-items-center">
            <h5 class="card-title fw-bold">Purchase Order {{ $po_header->pono }}</h5>
    ...
    <form id="form-edit-po" action="#" method="POST">
        @csrf
        <div class="card-body">
                ...
                <div class="col-md-3 form-group">
                    ...
                    <select name="currency" id="currency" class="form-control select2bs4">
                        ...
                        @foreach ($currency as $c)
                            <option value="{{ $c->id ?? $c->Id }}" {{ $currentCurr == ($c->id ?? $c->Id) ? 'selected' : '' }}>{{ $c->nama_pilihan ?? $c->Nama_Pilihan }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-3 form-group">
            ...
            <div class="table-responsive table-scroll-wrapper">
                <table class="table table-bordered table-custom" id="table-po-items">
                    ...
                    <tbody>
                        @forelse($po_items as $item)
                            @php
                                $consAsli = ($item->price_costing_conv > 0 && $item->convert_val > 0) ? ($item->price_costing * $item->convert_val / $item->price_costing_conv) : 1;
                                $displaySet = $item->product_set ? '<br><span class="badge badge-danger" style="font-size: 11px;">'.$item->product_set.'</span>' : '';
                            @endphp
                            <tr class="po-item-row" data-id="{{ $item->id_gen }}" data-set="{{ $item->product_set }}" data-cons-asli="{{ $consAsli }}">
                                ...
                                <td class="align-middle">
                                    <input type="hidden" name="id_item[]" value="{{ $item->id_gen }}">
            ...
            <div class="row align-items-end mb-3 bg-light p-3 rounded border mx-0">
                <div class="col-md-3 form-group mb-0">
                    <label><small class="fw-bold">Kategori :</small></label>
                     <select id="kategori_biaya" class="form-control select2bs4">
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-sm" id="table-biaya-lain">
                    ...
                    <tbody>
                        @foreach($po_biaya as $index => $b)
                        <tr class="bg-light">
                            <td class="text-center align-middle">{{ $index + 1 }}</td>
                            <td class="align-middle">
                                <input type="hidden" name="kategori_biaya[]" value="{{ $b->id_kategori }}">
                                {{ $b->nama_kategori }}
                            </td>
                            <td class="text-right align-middle">
                                <input type="hidden" name="value_biaya[]" class="val-subtotal" value="{{ rtrim(rtrim(number_format($b->total, 4, '.', ''), '0'), '.') }}">
                                {{ number_format($b->total, 2, ',', '.') }}
                            </td>
                            <td class="text-right align-middle">
                                <input type="hidden" name="is_ppn_biaya[]" value="{{ $b->ppn > 0 ? '1' : '0' }}">
                                <input type="hidden" name="ppn_biaya[]" class="val-ppn" value="{{ rtrim(rtrim(number_format($b->ppn, 4, '.', ''), '0'), '.') }}">
                                {{ number_format($b->ppn, 2, ',', '.') }}
                            </td>
                            <td class="text-right align-middle">
                                <input type="hidden" name="total_biaya[]" class="val-total" value="{{ rtrim(rtrim(number_format($b->total + $b->ppn, 4, '.', ''), '0'), '.') }}">
                                {{ number_format($b->total + $b->ppn, 2, ',', '.') }}
                            </td>
                            <td class="text-center align-middle">
    ...
    <div class="modal-dialog modal-xl modal-dialog-scrollable">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary btn-sm fw-bold" id="btn-add-item-to-table">Tambahkan</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    $(document).ready(function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if(response && response.length > 0) {
                        ...
                        tbody.html('<tr><td colspan="6" class="text-center font-italic text-danger">Material tidak ditemukan untuk jenis item ini.</td></tr>');
                    }
                }
            });
        });
        ...
        $('#form-edit-po').on('submit', function(e) {
            ...
            Swal.fire({
                title: 'Update Data PO?',
                text: "Data PO sebelumnya akan ditimpa dengan data terbaru.",
                icon: 'warning',
                showCancelButton: true,
                        ...
                        success: function(response) {
                            if(response.status === 200) {
                                Swal.fire('Berhasil!', response.message, 'success').then(() => {
                                    window.location.href = '{{ route("purchasing") }}';
                                });
                            } else {
