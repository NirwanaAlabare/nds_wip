            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <label>Sure Cancel Out Material Number :</label>
                        <input type="text" class="form-control text-center border-0"
                        id="txt_nodok" name="txt_nodok" readonly>

                        <div class="form-check mt-3">
                            <input class="form-check-input" type="checkbox" value="1" id="cancel_req" name="cancel_req">
                            <label class="form-check-label" for="cancel_req">
                                Sekalian Cancel Request (bppb_req)
                            </label>
                        </div>
...
<!-- <script src="{{ asset('plugins/ionicons/js/ionicons.esm.js') }}"></script>
    ...
    function approve_outmaterial(no_bppb) {
        Swal.fire({
            title: 'Cancel BPPB',
            text: "Pilih jenis pembatalan:",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonText: 'Cancel BPPB Only',
            cancelButtonText: 'Cancel + Material Request',
            reverseButtons: true
        }).then((result) => {
            if (result.isConfirmed) {
            // Cancel Only
            $.ajax({
                url: "{{ route('approve-outmaterial') }}",
                type: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
                    txt_nodok: no_bppb,
                ...
                success: function(res) {
                    Swal.fire('Berhasil', 'BPPB berhasil dicancel.', 'success');
                    location.reload();
                },
                error: function(err) {
            ...
            $.ajax({
                ...
                success: function(res) {
                    Swal.fire('Berhasil', 'BPPB dan Material Request dicancel.', 'success');
                    location.reload();
                },
                error: function(err) {
                    Swal.fire('Error', 'Terjadi kesalahan.', 'error');
