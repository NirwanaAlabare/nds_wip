    ...
    <form action="{{ route('export_excel_pemasukanroll') }}" method="get">
        <div class="card card-sb">
            <div class="card-header">
                <h5 class="card-title fw-bold mb-0"><i class="fas fa-file-alt fa-sm"></i> Laporan Pengeluaran Subcont Sewing</h5>
            </div>
            <div class="card-body">
...
@section('custom-script')
    ...
    <script>
        let datatable = $("#datatable").DataTable({
            ...
            scrollCollapse: true,
            ajax: {
                url: '{{ route('report-sewing-out-subcont') }}',
                data: function(d) {
                    d.dateFrom = $('#from').val();
                    d.dateTo = $('#to').val();
                },
    ...
    $.ajax({
        type: "get",
        url: '{{ route('export-excel-sewing-subcont-out') }}',
        data: {
            from: from,
            to: to
        },
