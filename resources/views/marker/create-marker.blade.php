...
@section('custom-script')
    ...
    <script>
        ...
        function remainingCutQty(orderQty, soDetId, panel) {
            // Get Total Cut Qty Based on Order WS, Order Color and Order Panel ( to know remaining cut qty )
            let sumCutQtyData = sumCutQty.find(o => o.so_det_id == soDetId && o.panel == panel)  ;
        ...
        let orderQtyDatatable = $("#orderQtyDatatable").DataTable({
            ...
            columns: [
                ...
                {
                    data: 'id' // detail so input
                },
                {
                    data: 'id' // ratio input
                },
                {
                    data: 'id' // cut qty input
                }
            ],
            columnDefs: [
                {
                ...
                },
                {
                    ...
                    render: (data, type, row, meta) => {
                        // Calculate Remaining Cut Qty
                        let remain = remainingCutQty(row.order_qty, row.id, $('#panel').val());
