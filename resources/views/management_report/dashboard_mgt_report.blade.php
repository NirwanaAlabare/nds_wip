...
@section('custom-link')
    ...
    <style>
        ...
        /* ---- Panel KPI khusus Full Earning: 2 kolom x 2 baris ---- */
        .fe-grid {
            display: grid;
        ...
        .fe-panel {
            ...
            border-left: 3px solid var(--fe-accent, #dee2e6);
            border-radius: 8px;
        }
        ...
        .fe-metric-value.v-earn {
            color: #28a745;
        }

        .fe-metric-value.v-cost {
            color: #dc3545;
        }

        .fe-metric-value.v-pos {
            color: #007bff;
        }

        .fe-metric-value.v-neg {
            color: #dc3545;
        }
    ...
    @if (in_array(auth()->user()->username, ['reza', 'admin_01', 'nirwana_it']))
        <div class="dash-wrap" id="dashWrap">
            ...
            <div class="d-flex align-items-center justify-content-between mb-3">
                <div>
                    <h5 class="dash-title"><i class="fas fa-chart-line mr-2" style="color:#007bff"></i>Earning vs Estimated
                        Cost
                    </h5>
            ...
            {{-- KPI Cards --}}
            <div class="kpi-grid mb-3" id="kpiGridMain">
            ...
            <div class="kpi-grid mb-3" id="kpiGridHighlight">
            ...
            {{-- KPI khusus Full Earning: 3 kolom x 4 baris, diisi renderFullEarnKpiGrid() --}}
            <div class="mb-3" id="kpiGridFullEarn" style="display:none;"></div>
            ...
            <div class="chart-card mb-3">
                ...
                <div id="noProductCostingResult"
                    style="display:none;text-align:center;color:#6c757d;padding:24px 0;font-size:0.85rem;">
                        ...
                        <div class="modal-body">
                            <div class="row">
                                <div class="col-6 mb-3">
                                    <label class="form-label" style="font-size:0.8rem;font-weight:600;">Tanggal
                                        Dari</label>
        ...
        <div id="syncOverlay">
            <div class="sync-box">
                ...
                <div style="font-size:0.78rem;color:#6c757d;">Mohon tunggu, proses ini bisa memakan waktu beberapa saat.
                </div>
            </div>
        </div>
    @endif
    ...
    @if (in_array(auth()->user()->username, ['reza', 'admin_01', 'nirwana_it']))
        ...
        <script>
            ...
            const PROD_EARN_REPORT = {
                ...
                /* field mana yang dipakai KPI Earning / Est Cost / Balance */
                metrics: {
                    earning: r => parseFloat(r.tot_earning_rupiah) || 0,
                    cost: r => parseFloat(r.est_tot_cost) || 0,
                    balance: r => parseFloat(r.blc) || 0,
                },
                ...
                applyUI: function() {
                    ...
                    /* dua baris KPI bawaan, grid full earning disembunyikan */
                    $('#kpiGridMain').show();
                    $('#kpiGridHighlight').show();
                    $('#kpiGridFullEarn').hide();
            ...
            const FULL_EARN_REPORT = {
                ...
                dailySeries: function(rows) {
                    const m = FULL_EARN_REPORT.metrics;
                    const byDay = {};
                    rows.forEach(function(r) {
                        ...
                        byDay[key].earning += m.earning(r);
                        byDay[key].cost += m.cost(r);
                        byDay[key].balance += m.balance(r);
                    });
                    return Object.values(byDay).sort((a, b) => a.tanggal.localeCompare(b.tanggal));
                },

                /* rekap harian tidak punya line/buyer, dikosongkan supaya filter & widget
                 * yang mengelompokkan per line/buyer tidak salah baca */
                normalizeRow: function(r) {
                ...
                /* KPI Earning / Est Cost / Balance memakai angka full earning
                 * dari mgt_rep_tmp_sum_prod_full_earning:
                 *   sum_est_full_earning - est_tot_cost = blc_full_earning */
                metrics: {
                    earning: r => parseFloat(r.sum_est_full_earning) || 0,
                    cost: r => parseFloat(r.est_tot_cost) || 0,
                    balance: r => parseFloat(r.blc_full_earning) || 0,
                },
                ...
                applyUI: function() {
                    ...
                    /* dua baris KPI bawaan diganti grid 3x4 khusus full earning */
                    $('#kpiGridMain').hide();
                ...
                /* KPI-nya punya grid sendiri, bukan card bawaan */
                renderKpi: function(rows) {
                    renderFullEarnKpiGrid(rows);
                },
            ...
            function renderDashboard() {
                ...
                report.renderKpi(rows);
                renderDailyChart(report.dailySeries(rows));
                report.render(rows);
            }
            ...
            function sumKpiTotals(rows) {
                ...
                // field earning/cost/balance beda per tipe report, lihat metrics di REPORTS
                const m = currentReport().metrics;

                rows.forEach(function(r) {
                    t.earning += m.earning(r);
                    t.cost += m.cost(r);
                    t.balance += m.balance(r);
            ...
            /* ---- KPI grid Full Earning: 3 kolom (Earning / Cost / Balance) x 4 baris ----
             * Satu baris = satu jenis earning di mgt_rep_tmp_sum_prod_full_earning. */
            const FULL_EARN_KPI_ROWS = [{
                    group: 'Aktual',
                    color: '#6c757d',
                    earning: 'sum_tot_earning_rupiah',
                    cost: 'est_tot_cost',
                    balance: 'blc',
                },
                {
                    group: 'Full Earning',
                    color: '#28a745',
                    earning: 'sum_est_full_earning',
                    cost: 'est_tot_cost',
                    balance: 'blc_full_earning',
                },
                {
                    group: 'Produksi',
                    color: '#fd7e14',
                    earning: 'sum_est_earning_prod',
                    cost: 'sum_est_cost_prod',
                    balance: 'blc_est_earn_cost_prod',
                },
                {
                    group: 'Marketing',
                    color: '#6f42c1',
                    earning: 'sum_est_earning_mkt',
                    cost: 'sum_est_cost_mkt',
                    balance: 'blc_est_earn_cost_mkt',
                },
            ];
            ...
            function fullEarnMetric(label, value, valueClass) {
                return `<div class="fe-metric">
                        <div class="fe-metric-label">${label}</div>
                        <div class="fe-metric-value ${valueClass}">${fmtRp(value)}</div>
                    </div>`;
            }

            function renderFullEarnKpiGrid(rows) {
                const $wrap = $('#kpiGridFullEarn').empty();
                const $grid = $('<div class="fe-grid"></div>').appendTo($wrap);

                FULL_EARN_KPI_ROWS.forEach(function(row) {
                    const earning = sumField(rows, row.earning);
                    const cost = sumField(rows, row.cost);
                    const balance = sumField(rows, row.balance);

                    const margin = cost > 0 ? (earning / cost) * 100 : 0;
                    const badge = cost > 0 ? margin.toFixed(1) + '%' : '—';

                    $grid.append(
                        `<div class="fe-panel" style="--fe-accent:${row.color}">
                        <div class="fe-panel-head">
                            <span class="fe-panel-title">${row.group}</span>
                            <span class="fe-panel-badge">${badge}</span>
                        </div>
                        <div class="fe-panel-body">
                            ${fullEarnMetric('Earning', earning, 'v-earn')}
                            ${fullEarnMetric('Cost', cost, 'v-cost')}
                            ${fullEarnMetric('Balance', balance, balance >= 0 ? 'v-pos' : 'v-neg')}
                        </div>
                    </div>`
                    );
                });
            ...
            function renderFullEarnBreakdownChart(rows) {
                ...
                rows.forEach(function(r) {
                    actualEarn += parseFloat(r.sum_tot_earning_rupiah) || 0;
                ...
                const opts = {
                    series: [{
                            name: 'Earning',
                            data: [Math.round(actualEarn), Math.round(fullEarn), Math.round(prodEarn), Math.round(
                                mktEarn)]
                        },
                        {
                            name: 'Cost',
                            data: [Math.round(actualCost), Math.round(fullCost), Math.round(prodCost), Math.round(
                                mktCost)]
                        },
                    ],
                    chart: {
