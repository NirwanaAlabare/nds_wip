                ...
                <div class="form-group row">
                    <div class="col-md-2">
                        <div class="mb-1">
                            ...
                            <input type="date" class="form-control form-control-sm" id="tgl_awal" name="tgl_awal" value="{{ date('Y-m-d') }}">
                        </div>
                    </div>
                    <div class="col-md-2">
                        <div class="mb-1">
                            <label class="form-label">To</label>
                            <input type="date" class="form-control form-control-sm" id="tgl_akhir" name="tgl_akhir" value="{{ date('Y-m-d') }}">
                        </div>
                    </div>
                    <div class="col-md-6" style="padding-top:0.5rem;">
                        <div class="mt-4">
                            ...
                            <a href="{{ route('create-sewing-out') }}" class="btn btn-sm btn-info"><i class="fas fa-plus"></i> Add Data</a>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row">
                    ...
                    <div class="col-6 mb-1"><strong>Status</strong><br><span id="d_status" class="badge bg-secondary">-</span></div>
                </div>
                <hr>
    ...
    columnDefs: [
        ...
        },
        {
            ...
            render: (data, type, row) => {
                ...
                let exportUrl = "{{ route('export-excel-sewing-out-detail', ':id') }}".replace(':id', row.id);
...
function showDetail(id) {
    ...
    $.get(url, function(res) {
        ...
        let badge  = status === 'APPROVED' ? 'bg-success' : (status === 'DRAFT' ? 'bg-warning text-dark' : 'bg-danger');
        ...
        detailDT = $("#detailTable").DataTable({
            searching: true, paging: true, ordering: true, info: false, lengthChange: false, pageLength: 10,
            columnDefs: [{ targets: 6, className: 'text-end', render: (d) => (parseFloat(d)||0).toLocaleString('en-US') }],
            footerCallback: function(row, data, start, end, display) {
                let api   = this.api();
                let total = api.column(6, { search: 'applied' }).data().reduce((a, b) => {
                    let toNum = v => typeof v === 'string' ? parseFloat(v.replace(/,/g,''))||0 : (typeof v === 'number' ? v : 0);
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                ...
                type: "POST",
                data: { _token: $('meta[name="csrf-token"]').attr('content'), no_bppb: no_bppb },
