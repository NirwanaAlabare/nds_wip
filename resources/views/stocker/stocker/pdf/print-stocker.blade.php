...
<body>
    ...
    @foreach ($dataStockers as $dataStocker)
        <table class="{{ $loop->index != 0 ? 'page-break' : '' }}">
            ...
            <tr>
                <td style="width: 50%; {{ strlen($dataStocker->size) > 13 ? 'font-size: 8px;' : '' }}">
                    Size : {{ strtoupper($dataStocker->size) }}
                </td>
                <td style="width: 50%; {{ strlen($dataStocker->dest) > 13 ? 'font-size: 8px;' : '' }}">
                    Country : {{ strtoupper($dataStocker->dest) }}
                </td>
            </tr>
            <tr>
        ...
        <table style="margin-top: -0.5px;">
            ...
            <tr>
                ...
                <td style="border: none;border-left: none; border-top: 0.75px solid; border-bottom: 0.75px solid;text-align: center;width: auto;">:</td>
                <td colspan="3" style="border: none;border-right: 0.75px solid; border-top: 0.75px solid; border-bottom: 0.75px solid;">{{ preg_replace('/}/', '', $dataStocker->shade) }}</td>
