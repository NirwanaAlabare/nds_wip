    ...
    <div class="card card-sb">
        ...
        <div class="card-body">
            ...
            <div class="row align-items-end">
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        <button class="btn btn-success btn-block mt-3" onclick="setMonthCountNumber()"><i class="fa fa-print"></i> Set Month Number</button>
                    </div>
                </div>
            </div>
            <div class="my-3">
...
@section('custom-script')
    ...
    <script>
        ...
        function getRangeMonthCount() {
            $.ajax({
                url: '{{ route('get-range-month-count') }}',
                type: 'get',
                data: {
        ...
        let monthCountTable = $("#month-count-table").DataTable({
            ordering: false,
            processing: true,
            serverSide: true,
            scrollX: '500px',
            ...
            ajax: {
                ...
                url: '{{ route('get-stocker-month-count') }}',
                dataType: 'json',
                dataSrc: 'data',
                data: function(d) {
        ...
        $('#month-count-table thead tr').clone(true).appendTo('#month-count-table thead');
        $('#month-count-table thead tr:eq(1) th').each(function(i) {
            var title = $(this).text();
            $(this).html('<input type="text" class="form-control form-control-sm" style="width:100%"/>');

            $('input', this).on('keyup change', function() {
                if (monthCountTable.column(i).search() !== this.value) {
                    monthCountTable
                        .column(i)
                        .search(this.value)
                        .draw();
        ...
        function monthCountTableReload() {
            $('#month-count-table').DataTable().ajax.reload();
        }
    </script>
@endsection
