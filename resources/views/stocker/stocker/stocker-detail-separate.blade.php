                        ...
                        <div id="separate_qty_wrapper_{{ $indexSeparate }}" class="d-flex flex-wrap gap-1"  data-qty="{{ $qty }}">
                            ...
                            @else
                                @for ($i = 0; $i < $ratio->ratio; $i++)
                                    <input type="number" class="form-control form-control-sm separate-part" name="separate_qty[{{ $indexSeparate }}][]" value="{{ $qty/$ratio->ratio }}" onkeyup="validateSeparateSum({{ $indexSeparate }})">
                                @endfor
                            @endif
                        </div>
                        <div class="mt-1">
